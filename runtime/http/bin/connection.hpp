...
class connection : public std::enable_shared_from_this<connection> {
 public:
  ...
  bool try_parse_headers() {
    ...
    //  解析内容信息
    if (content_length_ <= 0) content_length_ = parse_content_length(headers);
    // 解析HTTP版本
    ...
    if (expect_100_continue_) {
      ...
      if (http_version_minor_ < 1) send_417_expectation_failed();
      return true;
    }
    ...
    if (filename_.find(".wav") != std::string::npos) {
      std::cout << "set wav_format=pcm, file_name=" << filename_ << std::endl;
  ...
  void parse_multipart_boundary() {
    size_t content_type_pos =
        received_data_.find("Content-Type: multipart/form-data");
    if (content_type_pos == std::string::npos) return;
    ...
    boundary_ =
        received_data_.substr(boundary_pos, boundary_end - boundary_pos);
  ...
  void process_multipart_data() {
    ...
    while (true) {
      if (!in_file_part_) {
        ...
        part_start += 4;  // 跳过空行
        parse_part_headers(
            received_data_.substr(boundary_pos, part_start - boundary_pos));
        ...
        if (boundary_end == std::string::npos) break;

        // 写入内容

        std::string tmpstr = received_data_.substr(0, boundary_end);
        data_msg->samples->insert(data_msg->samples->end(), tmpstr.begin(),
                                  tmpstr.end());
  ...
  void parse_part_headers(const std::string &headers) {
    ...
    if (cl_pos != std::string::npos) {
      ...
      expected_part_size_ =
          std::stoull(headers.substr(cl_pos, cl_end - cl_pos));
    }
  }
 ...
 private:
  ...
  enum class State { ReadingHeaders, SendingContinue, ReadingBody };
