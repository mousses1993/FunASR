 ...
 public:
  ...
  void run(const std::string& uri, const std::vector<string>& wav_list,
    ...
    while (true) {
      ...
      total_send += 1;
      send_wav_data(wav_list[i], wav_ids[i], audio_fs, hws_map, send_hotword,
                    use_itn, svs_itn);
      if (send_hotword) {
        send_hotword = false;
      }
  ...
  void send_wav_data(string wav_path, string wav_id, int audio_fs,
    ...
    if (wav_format == "pcm") {
      while (audio.Fetch(buff, len, flag) > 0) {
        ...
        LOG(INFO) << "Thread: " << this_thread::get_id()
                  << ", sended data len=" << len * sizeof(short);
...
#include <windows.h>
  ...
  for (size_t i = 0; i < threads_num; i++) {
    client_threads.emplace_back([uri, wav_list, wav_ids, audio_fs, is_ssl,
