...
void WebSocketServer::do_decoder(
    const std::vector<char>& buffer, websocketpp::connection_hdl& hdl,
    nlohmann::json& msg, websocketpp::lib::mutex& thread_lock,
    std::vector<std::vector<float>>& hotwords_embedding, std::string wav_name,
    bool itn, int audio_fs, std::string wav_format,
    FUNASR_DEC_HANDLE& decoder_handle, std::string svs_lang, bool sys_itn) {
  try {
...
void WebSocketServer::on_open(websocketpp::connection_hdl hdl) {
  ...
  FUNASR_DEC_HANDLE decoder_handle = FunASRWfstDecoderInit(
      asr_handle, ASR_OFFLINE, global_beam_, lattice_beam_, am_scale_);
    ...
    case websocketpp::frame::opcode::text: {
      ...
      if (msg_data->hotwords_embedding == nullptr) {
        ...
        std::vector<std::vector<float>> new_hotwords_embedding =
            CompileHotwordEmbedding(asr_handle, nn_hotwords);
      ...
      if ((jsonresult["is_speaking"] == false ||
        ...
        asio::post(
            ...
            std::bind(&WebSocketServer::do_decoder, this,
                      ...
                      std::ref(msg_data->msg), std::ref(*thread_lock_p),
                      std::move(hotwords_embedding_), msg_data->msg["wav_name"],
                      msg_data->msg["itn"], msg_data->msg["audio_fs"],
                      msg_data->msg["wav_format"],
                      std::ref(msg_data->decoder_handle),
                      msg_data->msg["svs_lang"], msg_data->msg["svs_itn"]));
