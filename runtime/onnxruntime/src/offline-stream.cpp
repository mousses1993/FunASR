...
namespace funasr {
OfflineStream::OfflineStream(std::map<std::string, std::string>& model_path,
  ...
  if (model_path.find(MODEL_DIR) != model_path.end()) {
...
#else
      LOG(ERROR) << "GPU is not supported! CPU will be used! If you want to "
                    "use GPU, please add -DGPU=ON when cmake";
    ...
    hw_gpu_model_path =
        PathAppend(model_path.at(MODEL_DIR), TORCH_MODEL_EB_NAME);
    seg_dict_path = PathAppend(model_path.at(MODEL_DIR), MODEL_SEG_DICT);
    if (access(hw_cpu_model_path.c_str(), F_OK) ==
    ...
    if (use_gpu && access(hw_gpu_model_path.c_str(), F_OK) ==
                       0) {  // if model_eb.torchscript exist, hotword enabled
      enable_hotword = true;
    ...
    if (use_gpu) {
      ...
      if (model_path.find(BLADEDISC) != model_path.end() &&
          model_path.at(BLADEDISC) == "true") {
    ...
    asr_handle->InitAsr(am_model_path, am_cmvn_path, am_config_path, token_path,
                        thread_num);
  }
  ...
  if (model_path.find(PUNC_DIR) != model_path.end()) {
    ...
    if (access(punc_model_path.c_str(), F_OK) != 0 ||
      ...
      punc_handle->InitPunc(punc_model_path, punc_config_path, token_path,
                            thread_num);
      use_punc = true;
    }
  }
