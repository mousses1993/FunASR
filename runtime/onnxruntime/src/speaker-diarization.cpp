...
SpectralClustering::SpectralClustering(int min_num_spks, int max_num_spks,
                                       float pval)
...
static std::vector<float> PowerIteration(
    const std::vector<std::vector<float>>& matrix, int max_iterations = 100,
...
static int EstimateNumSpeakersByEigengap(const std::vector<float>& eigenvalues,
  ...
  // Sort eigenvalues in descending order (they should already be sorted by
  // power iteration) Find the largest gap between consecutive eigenvalues
  float max_gap = 0.0f;
  int optimal_k = min_spks;

  int check_limit =
      std::min(static_cast<int>(eigenvalues.size()), max_spks + 1);
...
SpectralClustering::GetSpectralEmbeddings(
  ...
  if (oracle_num > 0) {
    ...
    int num_eigenvalues_to_compute =
        std::min(static_cast<int>(n), max_num_spks_ + 3);
    auto eigenvalues =
        ComputeEigenvalues(laplacian, num_eigenvalues_to_compute);

    if (eigenvalues.size() >= 2) {
      num_speakers = EstimateNumSpeakersByEigengap(eigenvalues, min_num_spks_,
                                                   max_num_spks_);
    } else {
  ...
  std::vector<std::vector<float>> embeddings(
      n, std::vector<float>(num_speakers, 0.0f));
  for (size_t i = 0; i < n; ++i) {
...
std::vector<int> SpectralClustering::KMeansClustering(
  ...
  // Step 2: Choose remaining centroids with probability proportional to
  // distance squared
  for (int c = 1; c < k; ++c) {
...
bool SpeakerDiarization::Init(
  ...
  if (segment_duration_ <= 0.0f) {
    LOG(ERROR) << "Invalid segment_duration: " << segment_duration_
               << ", using default";
    segment_duration_ = DEFAULT_SEGMENT_DURATION;
  }
  if (segment_shift_ <= 0.0f) {
    LOG(ERROR) << "Invalid segment_shift: " << segment_shift_
               << ", using default";
    segment_shift_ = DEFAULT_SEGMENT_SHIFT;
  }
  if (min_num_speakers_ < 1) {
    LOG(ERROR) << "Invalid min_num_speakers: " << min_num_speakers_
               << ", using default";
...
void SpeakerDiarization::AssignSpeakersToSentences(
  ...
  for (auto& sentence : sentence_list) {
    ...
    for (const auto& spk_seg : speaker_segments) {
      float overlap =
          std::max(0.0f, std::min(sent_end, spk_seg.end_time) -
                             std::max(sent_start, spk_seg.start_time));
...
SpeakerDiarization::ChunkSegments(
    const std::vector<std::tuple<float, float, std::vector<float>>>&
        vad_segments,
    int sample_rate) {
  ...
  for (const auto& seg : vad_segments) {
    ...
    for (int chunk_start = 0; chunk_start < data_len;
      ...
      float chunk_time_start =
          actual_start / static_cast<float>(sample_rate) + seg_start;
      float chunk_time_end =
          chunk_end / static_cast<float>(sample_rate) + seg_start;
