...
ParaformerOnline::ParaformerOnline(Model *offline_handle,
                                   std::vector<int> chunk_size,
                                   std::string model_type)
    : offline_handle_(std::move(offline_handle)),
      chunk_size(chunk_size),
      session_options_{} {
  if (model_type == MODEL_PARA) {
    Paraformer *para_handle = dynamic_cast<Paraformer *>(offline_handle_);
    InitOnline(para_handle->fbank_opts_, para_handle->encoder_session_,
               para_handle->decoder_session_, para_handle->en_szInputNames_,
               para_handle->en_szOutputNames_, para_handle->de_szInputNames_,
               para_handle->de_szOutputNames_, para_handle->means_list_,
               para_handle->vars_list_, para_handle->frame_length,
               para_handle->frame_shift, para_handle->n_mels,
               para_handle->lfr_m, para_handle->lfr_n,
               para_handle->encoder_size, para_handle->fsmn_layers,
               para_handle->fsmn_lorder, para_handle->fsmn_dims,
               para_handle->cif_threshold, para_handle->tail_alphas);
  } else if (model_type == MODEL_SVS) {
    SenseVoiceSmall *svs_handle =
        dynamic_cast<SenseVoiceSmall *>(offline_handle_);
    InitOnline(svs_handle->fbank_opts_, svs_handle->encoder_session_,
               svs_handle->decoder_session_, svs_handle->en_szInputNames_,
               svs_handle->en_szOutputNames_, svs_handle->de_szInputNames_,
               svs_handle->de_szOutputNames_, svs_handle->means_list_,
               svs_handle->vars_list_, svs_handle->frame_length,
               svs_handle->frame_shift, svs_handle->n_mels, svs_handle->lfr_m,
               svs_handle->lfr_n, svs_handle->encoder_size,
               svs_handle->fsmn_layers, svs_handle->fsmn_lorder,
               svs_handle->fsmn_dims, svs_handle->cif_threshold,
...
void ParaformerOnline::InitOnline(
    ...
    vector<const char *> &de_szOutputNames, vector<float> &means_list,
    vector<float> &vars_list, int frame_length_, int frame_shift_, int n_mels_,
    int lfr_m_, int lfr_n_, int encoder_size_, int fsmn_layers_,
    int fsmn_lorder_, int fsmn_dims_, float cif_threshold_,
    float tail_alphas_) {
  ...
  chunk_len = chunk_size[1] * frame_shift * lfr_n *
              offline_handle_->GetAsrSampleRate() / 1000;

  frame_sample_length_ =
      offline_handle_->GetAsrSampleRate() / 1000 * frame_length;
  frame_shift_sample_length_ =
      offline_handle_->GetAsrSampleRate() / 1000 * frame_shift;
}

void ParaformerOnline::FbankKaldi(float sample_rate,
  ...
  int frame_number = ComputeFrameNum(waves.size(), frame_sample_length_,
                                     frame_shift_sample_length_);
  // Send the audio after the last frame shift position to the cache
  input_cache_.clear();
  input_cache_.insert(input_cache_.begin(),
                      waves.begin() + frame_number * frame_shift_sample_length_,
                      waves.end());
  if (frame_number == 0) {
    return;
  }
  // Delete audio that haven't undergone fbank processing
  waves.erase(waves.begin() + (frame_number - 1) * frame_shift_sample_length_ +
                  frame_sample_length_,
              waves.end());
...
void ParaformerOnline::ExtractFeats(float sample_rate,
                                    vector<std::vector<float>> &wav_feats,
  ...
  if (wav_feats.size() > 0) {
    ...
    if (wav_feats.size() + lfr_splice_cache_.size() >= lfr_m) {
      wav_feats.insert(wav_feats.begin(), lfr_splice_cache_.begin(),
                       lfr_splice_cache_.end());
      int frame_from_waves =
          (waves.size() - frame_sample_length_) / frame_shift_sample_length_ +
          1;
      ...
      int sample_length = (frame_from_waves - 1) * frame_shift_sample_length_ +
                          frame_sample_length_;
      ...
      reserve_waveforms_.insert(
          reserve_waveforms_.begin(),
          waves.begin() + frame_sample_length_ - frame_shift_sample_length_,
          waves.end());
      lfr_splice_cache_.insert(lfr_splice_cache_.end(), wav_feats.begin(),
                               wav_feats.end());
    }
  } else {
    if (input_finished) {
...
int ParaformerOnline::OnlineLfrCmvn(vector<vector<float>> &wav_feats,
                                    bool input_finished) {
  ...
  for (int i = 0; i < T_lrf; i++) {
    if (lfr_m <= T - i * lfr_n) {
      for (int j = 0; j < lfr_m; j++) {
        p.insert(p.end(), wav_feats[i * lfr_n + j].begin(),
                 wav_feats[i * lfr_n + j].end());
      }
      ...
      if (input_finished) {
        ...
        for (int j = 0; j < num_padding; j++) {
          p.insert(p.end(), wav_feats[wav_feats.size() - 1].begin(),
                   wav_feats[wav_feats.size() - 1].end());
        }
        out_feats.emplace_back(p);
        p.clear();
      } else {
  ...
  lfr_splice_cache_.insert(lfr_splice_cache_.begin(),
                           wav_feats.begin() + lfr_splice_frame_idxs,
                           wav_feats.end());
...
void ParaformerOnline::GetPosEmb(std::vector<std::vector<float>> &wav_feats,
                                 int timesteps, int feat_dim) {
...
void ParaformerOnline::CifSearch(std::vector<std::vector<float>> hidden,
                                 std::vector<float> alphas, bool is_final,
  ...
  try {
    ...
    for (i = 0; i < chunk_size_pre; i++) alphas[i] = 0.0;

    int chunk_size_suf =
        std::accumulate(chunk_size.begin(), chunk_size.end() - 1, 0);
...
void ParaformerOnline::InitCache() {
  ...
  for (int l = 0; l < fsmn_layers; l++) {
    Ort::Value onnx_fsmn_cache = Ort::Value::CreateTensor<float>(
        m_memoryInfo, fsmn_init_cache_.data(), fsmn_init_cache_.size(),
        fsmn_shape_, 3);
    decoder_onnx.emplace_back(std::move(onnx_fsmn_cache));
  }
};

void ParaformerOnline::Reset() { InitCache(); }
...
void ParaformerOnline::AddOverlapChunk(
    std::vector<std::vector<float>> &wav_feats, bool input_finished) {
  wav_feats.insert(wav_feats.begin(), feats_cache_.begin(), feats_cache_.end());
  if (input_finished) {
    feats_cache_.clear();
    feats_cache_.insert(feats_cache_.begin(), wav_feats.end() - chunk_size[0],
                        wav_feats.end());
    if (!is_last_chunk) {
      int padding_length =
          std::accumulate(chunk_size.begin(), chunk_size.end(), 0) -
          wav_feats.size();
    ...
    feats_cache_.clear();
    feats_cache_.insert(feats_cache_.begin(),
                        wav_feats.end() - chunk_size[0] - chunk_size[2],
                        wav_feats.end());
  }
}

string ParaformerOnline::ForwardChunk(
    std::vector<std::vector<float>> &chunk_feats, bool input_finished) {
  string result;
  try {
    ...
    Ort::Value onnx_feats_len = Ort::Value::CreateTensor<int32_t>(
        m_memoryInfo, paraformer_length.data(), paraformer_length.size(),
        paraformer_length_shape, 1);

    std::vector<Ort::Value> input_onnx;
    input_onnx.emplace_back(std::move(onnx_feats));
    input_onnx.emplace_back(std::move(onnx_feats_len));

    auto encoder_tensor = encoder_session_->Run(
        Ort::RunOptions{nullptr}, en_szInputNames_.data(), input_onnx.data(),
        input_onnx.size(), en_szOutputNames_.data(), en_szOutputNames_.size());

    // get enc_vec
    std::vector<int64_t> enc_shape =
        encoder_tensor[0].GetTensorTypeAndShapeInfo().GetShape();
    float *enc_data = encoder_tensor[0].GetTensorMutableData<float>();
    std::vector<std::vector<float>> enc_vec(enc_shape[1],
                                            std::vector<float>(enc_shape[2]));
    ...
    std::vector<int64_t> alpha_shape =
        encoder_tensor[2].GetTensorTypeAndShapeInfo().GetShape();
    ...
    if (list_frame.size() > 0) {
      ...
      // enc_lens
      decoder_onnx.insert(decoder_onnx.begin() + 1,
                          std::move(encoder_tensor[1]));

      // acoustic_embeds
      const int64_t emb_shape_[3] = {1, (int64_t)list_frame.size(),
                                     (int64_t)list_frame[0].size()};
      std::vector<float> emb_input;
      for (const auto &list_frame_ : list_frame) {
        emb_input.insert(emb_input.end(), list_frame_.begin(),
                         list_frame_.end());
      }
      ...
      Ort::Value onnx_emb_len = Ort::Value::CreateTensor<int32_t>(
          m_memoryInfo, emb_length.data(), emb_length.size(), emb_length_shape,
          1);
      decoder_onnx.insert(decoder_onnx.begin() + 3, std::move(onnx_emb_len));

      auto decoder_tensor = decoder_session_->Run(
          Ort::RunOptions{nullptr}, de_szInputNames_.data(),
          decoder_onnx.data(), decoder_onnx.size(), de_szOutputNames_.data(),
          de_szOutputNames_.size());
      // fsmn cache
      try {
        decoder_onnx.clear();
      } catch (std::exception const &e) {
      ...
      std::vector<int64_t> decoder_shape =
          decoder_tensor[0].GetTensorTypeAndShapeInfo().GetShape();
      float *float_data = decoder_tensor[0].GetTensorMutableData<float>();
      result = offline_handle_->GreedySearch(float_data, list_frame.size(),
                                             decoder_shape[2]);
    }
  } catch (std::exception const &e) {
    LOG(ERROR) << e.what();
...
string ParaformerOnline::Forward(float *din, int len, bool input_finished,
  ...
  try {
    ...
    ExtractFeats(offline_handle_->GetAsrSampleRate(), wav_feats, waves,
                 input_finished);
    if (wav_feats.size() == 0) {
      return result;
    }
    ...
    if (input_finished) {
      if (wav_feats.size() + chunk_size[2] <= chunk_size[1]) {
        ...
        first_chunk.insert(first_chunk.begin(), wav_feats.begin(),
                           wav_feats.end());
        ...
        last_chunk.insert(last_chunk.begin(),
                          wav_feats.end() - (wav_feats.size() + chunk_size[2] -
                                             chunk_size[1]),
                          wav_feats.end());
