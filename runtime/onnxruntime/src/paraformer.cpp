...
Paraformer::Paraformer()
    : use_hotword(false),
      env_(ORT_LOGGING_LEVEL_ERROR, "paraformer"),
      session_options_{},
      hw_env_(ORT_LOGGING_LEVEL_ERROR, "paraformer_hw"),
      hw_session_options{} {}

// offline
void Paraformer::InitAsr(const std::string &am_model,
                         const std::string &am_cmvn,
                         const std::string &am_config,
                         const std::string &token_file, int thread_num) {
  ...
  try {
    m_session_ = std::make_unique<Ort::Session>(
        env_, ORTSTRING(am_model).c_str(), session_options_);
...
// online
void Paraformer::InitAsr(const std::string &en_model,
                         const std::string &de_model,
                         const std::string &am_cmvn,
                         const std::string &am_config,
                         const std::string &token_file, int thread_num) {
  ...
  try {
    encoder_session_ = std::make_unique<Ort::Session>(
        env_, ORTSTRING(en_model).c_str(), session_options_);
  ...
  try {
    decoder_session_ = std::make_unique<Ort::Session>(
        env_, ORTSTRING(de_model).c_str(), session_options_);
  ...
  for (auto &item : en_strInputNames) en_szInputNames_.push_back(item.c_str());
  ...
  for (auto &item : de_strInputNames) de_szInputNames_.push_back(item.c_str());
...
// 2pass
void Paraformer::InitAsr(const std::string &am_model,
                         const std::string &en_model,
                         const std::string &de_model,
                         const std::string &am_cmvn,
                         const std::string &am_config,
                         const std::string &token_file,
                         const std::string &online_token_file, int thread_num) {
  // online
  InitAsr(en_model, de_model, am_cmvn, am_config, online_token_file,
          thread_num);
...
void Paraformer::InitHwCompiler(const std::string &hw_model, int thread_num) {
  ...
  try {
    ...
    LOG(WARNING)
        << "Failed to enable CUDA Execution Provider for hotword compiler: "
        << e.what() << ", falling back to CPU";
  }
#endif

  try {
    hw_m_session = std::make_unique<Ort::Session>(
        hw_env_, ORTSTRING(hw_model).c_str(), hw_session_options);
...
void Paraformer::FbankKaldi(float sample_rate, const float *waves, int len,
...
void Paraformer::LoadCmvn(const char *filename) {
  ...
  while (getline(cmvn_stream, line)) {
    ...
    vector<string> line_item{istream_iterator<string>{iss},
...
string Paraformer::GreedySearch(float *in, int n_len, int64_t token_nums,
                                bool is_stamp, std::vector<float> us_alphas,
...
string Paraformer::BeamSearch(WfstDecoder *&wfst_decoder, float *in, int len,
                              int64_t token_nums) {
  return wfst_decoder->Search(in, len, token_nums);
}

string Paraformer::FinalizeDecode(WfstDecoder *&wfst_decoder, bool is_stamp,
  ...
  for (int i = 0; i < T_lrf; i++) {
    if (lfr_m <= T - i * lfr_n) {
      for (int j = 0; j < lfr_m; j++) {
        p.insert(p.end(), asr_feats[i * lfr_n + j].begin(),
                 asr_feats[i * lfr_n + j].end());
      }
      ...
      for (int j = 0; j < num_padding; j++) {
        p.insert(p.end(), asr_feats[asr_feats.size() - 1].begin(),
                 asr_feats[asr_feats.size() - 1].end());
      }
      out_feats.emplace_back(p);
      p.clear();
    }
...
std::vector<std::string> Paraformer::Forward(
    float **din, int *len, bool input_finished,
    const std::vector<std::vector<float>> &hw_emb, void *decoder_handle,
    int batch_in) {
  ...
  try {
    if (use_hotword) {
      ...
      const int64_t hotword_shape[3] = {1, static_cast<int64_t>(hw_emb.size()),
                                        static_cast<int64_t>(hw_emb[0].size())};
      embedding.reserve(hw_emb.size() * hw_emb[0].size());
      for (auto item : hw_emb) {
        embedding.insert(embedding.end(), item.begin(), item.end());
      }
      // LOG(INFO) << "hotword shape " << hotword_shape[0] << " " <<
      // hotword_shape[1] << " " << hotword_shape[2] << " size " <<
      // embedding.size();
  ...
  try {
    auto outputTensor = m_session_->Run(
        Ort::RunOptions{nullptr}, m_szInputNames.data(), input_onnx.data(),
        input_onnx.size(), m_szOutputNames.data(), m_szOutputNames.size());
    std::vector<int64_t> outputShape =
        outputTensor[0].GetTensorTypeAndShapeInfo().GetShape();
    // LOG(INFO) << "paraformer out shape " << outputShape[0] << " " <<
    ...
    if (outputTensor.size() == 4) {
      std::vector<int64_t> us_alphas_shape =
          outputTensor[2].GetTensorTypeAndShapeInfo().GetShape();
      ...
      std::vector<int64_t> us_peaks_shape =
          outputTensor[3].GetTensorTypeAndShapeInfo().GetShape();
      ...
      if (lm_ == nullptr) {
        result = GreedySearch(floatData, *encoder_out_lens, outputShape[2],
                              true, us_alphas, us_peaks);
      } else {
        result = BeamSearch(wfst_decoder, floatData, *encoder_out_lens,
                            outputShape[2]);
        if (input_finished) {
...
std::vector<std::vector<float>> Paraformer::CompileHotwordEmbedding(
    std::string &hotwords) {
  ...
  if (!hotwords.empty()) {
    ...
    for (auto hotword : hotword_array) {
      ...
      if (EncodeConverter::IsAllChineseCharactor(
              (const U8CHAR_T *)hotword.c_str(), hotword.size())) {
      ...
      real_hw_size += 1;
      hotword_matrix.insert(hotword_matrix.end(), hw_vector.begin(),
                            hw_vector.end());
    }
  ...
  blank_vec[0] = 1;
  hotword_matrix.insert(hotword_matrix.end(), blank_vec.begin(),
                        blank_vec.end());
  lengths.push_back(1);
  ...
  Ort::Value onnx_hotword = Ort::Value::CreateTensor<int32_t>(
      m_memoryInfo, (int32_t *)hotword_matrix.data(),
      hotword_size * max_hotword_len, input_shape_, 2);
  LOG(INFO) << "clas shape " << hotword_size << " " << max_hotword_len
            << std::endl;
  ...
  try {
    auto outputTensor =
        hw_m_session->Run(Ort::RunOptions{nullptr}, hw_m_szInputNames.data(),
                          input_onnx.data(), input_onnx.size(),
    ...
    float *floatData =
        outputTensor[0]
            .GetTensorMutableData<float>();  // shape [max_hotword_len,
                                             // hotword_size, dim]
    ...
    for (int j = 0; j < hotword_size; j++) {
      int start_pos =
          hotword_size * (lengths[j] - 1) * embedding_dim + j * embedding_dim;
      std::vector<float> embedding;
      embedding.insert(embedding.begin(), floatData + start_pos,
                       floatData + start_pos + embedding_dim);
...
Vocab *Paraformer::GetVocab() { return vocab; }

Vocab *Paraformer::GetLmVocab() { return lm_vocab; }

PhoneSet *Paraformer::GetPhoneSet() { return phone_set_; }
