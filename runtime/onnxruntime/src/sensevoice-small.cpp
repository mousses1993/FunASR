...
SenseVoiceSmall::SenseVoiceSmall()
    : use_hotword(false),
      env_(ORT_LOGGING_LEVEL_ERROR, "sensevoice"),
      session_options_{} {}

// offline
void SenseVoiceSmall::InitAsr(const std::string &am_model,
...
// online
void SenseVoiceSmall::InitAsr(const std::string &en_model,
...
// 2pass
void SenseVoiceSmall::InitAsr(
    const std::string &am_model, const std::string &en_model,
    const std::string &de_model, const std::string &am_cmvn,
...
void SenseVoiceSmall::FbankKaldi(float sample_rate, const float *waves, int len,
...
string SenseVoiceSmall::CTCSearch(float *in,
  ...
  for (int32_t t = 0; t != paraformer_length[0]; ++t) {
    auto y = std::distance(
        ...
        std::max_element(static_cast<const float *>(in),
...
string SenseVoiceSmall::GreedySearch(float *in, int n_len, int64_t token_nums,
                                     bool is_stamp,
...
std::vector<std::vector<float>> SenseVoiceSmall::CompileHotwordEmbedding(
    std::string &hotwords) {
...
std::vector<std::string> SenseVoiceSmall::Forward(float **din, int *len,
                                                  bool input_finished,
                                                  std::string svs_lang,
                                                  bool svs_itn, int batch_in) {
  ...
  Ort::Value onnx_itn = Ort::Value::CreateTensor<int32_t>(
      m_memoryInfo, textnorm_length.data(), textnorm_length.size(),
      textnorm_shape, 1);
