...
void KeepChineseCharacterAndSplit(
...
bool TimestampIsDigit(U16CHAR_T &u16) { return u16 >= L'0' && u16 <= L'9'; }
...
std::string TimestampSmooth(std::string &text, std::string &text_itn,
                            std::string &str_time) {
    ...
    if (alignment_str1[index] == alignment_str2[index]) {
      ...
      if (itn_count > 0 && timestamps_tmp.size() == 0) {
        if (idx_tp >= timestamps.size()) {
          LOG(ERROR)
              << "Timestamp Smooth Failed: Index of tp is out of range. ";
          return timestamps_str;
        }
...
size_t Utf8ToCharset(const std::string &input,
  ...
  for (i = 0; i < raw_char.size(); i++) {
    ...
    // step2 combie phoneme to full word
    {
      ...
      if (sub_word) {
        ...
        if (i == raw_char.size() - 1 ||
            i < raw_char.size() - 1 && IsChinese(raw_char[i + 1])) {
    ...
    // step3 process english word deal with space , turn abbreviation to upper
    // case
    {
...
void TimestampOnnx(std::vector<float> &us_alphas,
                   ...
                   std::vector<string> &char_list, std::string &res_str,
                   std::vector<std::vector<float>> &timestamp_vec,
                   float begin_time, float total_offset) {
  if (char_list.empty()) {
    return;
  }
  ...
  // for bicif model trained with large data, cif2 actually fires when a
  // character starts so treat the frames between two peaks as the duration of
  // the former token
  ...
  for (int i = 0; i < num_peak - 1; i++) {
    ...
    if (i == num_peak - 2 || MAX_TOKEN_DURATION < 0 ||
        fire_place[i + 1] - fire_place[i] < MAX_TOKEN_DURATION) {
      timestamp_list.push_back(
      ...
      timestamp_list.push_back(
          {_split * TIME_RATE, fire_place[i + 1] * TIME_RATE});
      new_char_list.push_back("<sil>");
    }
  }
  ...
  for (int i = 0; i < (int)new_char_list.size(); i++) {
    res_str += new_char_list[i] + " " + to_string(timestamp_list[i][0]) + " " +
               to_string(timestamp_list[i][1]) + ";";
  }
...
void SmoothTimestamps(std::string &str_punc, std::string &str_itn,
                      std::string &str_timetamp) {
  return;
}
