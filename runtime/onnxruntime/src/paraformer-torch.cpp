...
// offline
void ParaformerTorch::InitAsr(const std::string &am_model,
...
void ParaformerTorch::InitHwCompiler(const std::string &hw_model,
                                     int thread_num) {
  // TODO
  torch::DeviceType device = at::kCPU;
#ifdef USE_GPU
...
void ParaformerTorch::FbankKaldi(float sample_rate, const float *waves, int len,
...
string ParaformerTorch::BeamSearch(WfstDecoder *&wfst_decoder, float *in,
                                   int len, int64_t token_nums) {
...
std::vector<std::string> ParaformerTorch::Forward(
  ...
  for (int index = 0; index < batch_in; index++) {
    ...
    std::memcpy(&all_feats[index * max_frames * feature_dim],
                feats_batch[index].data(),
                max_frames * feature_dim * sizeof(float));
  }
  torch::Tensor feats =
      torch::from_blob(all_feats.data(), {batch_in, max_frames, feature_dim},
                       torch::kFloat)
          .contiguous();
  torch::Tensor feat_lens =
      torch::from_blob(paraformer_length.data(), {batch_in}, torch::kInt32);
  ...
  try {
    if (use_hotword) {
      ...
      for (size_t index = 0; index < batch_in; ++index) {
        batch_embedding.insert(batch_embedding.end(), embedding.begin(),
                               embedding.end());
      }

      torch::Tensor tensor_hw_emb =
          torch::from_blob(batch_embedding.data(),
                           {batch_in, static_cast<int64_t>(hw_emb.size()),
  ...
  try {
    ...
    for (int index = 0; index < batch_in; index++) {
      ...
      if (outputs.size() == 4) {
        ...
        if (lm_ == nullptr) {
          result = GreedySearch(am_scores[index].data_ptr<float>(),
                                valid_token_lens[index].item<int>(),
                                am_scores.size(2), true, us_alphas, us_peaks);
        } else {
          result = BeamSearch(wfst_decoder, am_scores[index].data_ptr<float>(),
                              valid_token_lens[index].item<int>(),
                              am_scores.size(2));
          if (input_finished) {
...
void ParaformerTorch::WarmUp() {
  ...
  if (use_hotword) {
    ...
    std::vector<std::vector<float>> hw_emb =
        CompileHotwordEmbedding(hotwords_wp);
    std::vector<float> embedding;
    embedding.reserve(hw_emb.size() * hw_emb[0].size());
    for (auto item : hw_emb) {
      embedding.insert(embedding.end(), item.begin(), item.end());
...
std::vector<std::vector<float>> ParaformerTorch::CompileHotwordEmbedding(
    std::string &hotwords) {
  ...
  torch::Tensor feats =
      torch::from_blob(hotword_matrix.data(), {hotword_size, max_hotword_len},
                       torch::kInt32)
          .contiguous();
...
Vocab *ParaformerTorch::GetVocab() { return vocab; }

Vocab *ParaformerTorch::GetLmVocab() { return lm_vocab; }

PhoneSet *ParaformerTorch::GetPhoneSet() { return phone_set_; }
