...
class SenseVoiceSmall : public Model {
 ...
 public:
  ...
  std::vector<std::string> Forward(float **din, int *len,
                                   ...
                                   bool svs_itn = true, int batch_in = 1);
  string CTCSearch(float *in, std::vector<int32_t> paraformer_length,
  ...
  // const std::string &lex_file); string BeamSearch(WfstDecoder* &wfst_decoder,
  // float* in, int n_len, int64_t token_nums); string
  // FinalizeDecode(WfstDecoder* &wfst_decoder,
  //                   bool is_stamp=false, std::vector<float> us_alphas={0},
  //                   std::vector<float> us_cif_peak={0});
  // Vocab* GetVocab();
  // Vocab* GetLmVocab();
  ...
  // dict
  std::map<std::string, int> lid_map = {{"auto", 0},     {"zh", 3},  {"en", 4},
                                        {"yue", 7},      {"ja", 11}, {"ko", 12},
                                        {"nospeech", 13}};
};
