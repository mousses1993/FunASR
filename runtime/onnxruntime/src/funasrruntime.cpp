...
FunASRInit(std::map<std::string, std::string>& model_path, int thread_num,
           ASR_TYPE type) {
...
_FUNASRAPI FUNASR_HANDLE
FsmnVadInit(std::map<std::string, std::string>& model_path, int thread_num) {
...
_FUNASRAPI FUNASR_HANDLE
CTTransformerInit(std::map<std::string, std::string>& model_path,
                  int thread_num, PUNC_TYPE type) {
  funasr::PuncModel* mm = funasr::CreatePuncModel(model_path, thread_num, type);
  return mm;
}

_FUNASRAPI FUNASR_HANDLE
FunOfflineInit(std::map<std::string, std::string>& model_path, int thread_num,
               bool use_gpu, int batch_size) {
  funasr::OfflineStream* mm =
      funasr::CreateOfflineStream(model_path, thread_num, use_gpu, batch_size);
  return mm;
}

_FUNASRAPI FUNASR_HANDLE
FunTpassInit(std::map<std::string, std::string>& model_path, int thread_num) {
  funasr::TpassStream* mm = funasr::CreateTpassStream(model_path, thread_num);
  return mm;
}

_FUNASRAPI FUNASR_HANDLE FunTpassOnlineInit(FUNASR_HANDLE tpass_handle,
                                            std::vector<int> chunk_size) {
  return funasr::CreateTpassOnlineStream(tpass_handle, chunk_size);
}

// APIs for ASR Infer
_FUNASRAPI FUNASR_RESULT FunASRInferBuffer(
    FUNASR_HANDLE handle, const char* sz_buf, int n_len, FUNASR_MODE mode,
    QM_CALLBACK fn_callback, bool input_finished, int sampling_rate,
    std::string wav_format) {
  funasr::Model* recog_obj = (funasr::Model*)handle;
  if (!recog_obj) return nullptr;

  funasr::Audio audio(recog_obj->GetAsrSampleRate(), 1);
  if (wav_format == "pcm" || wav_format == "PCM") {
    if (!audio.LoadPcmwav(sz_buf, n_len, &sampling_rate)) return nullptr;
  } else {
    if (!audio.FfmpegLoad(sz_buf, n_len)) return nullptr;
  }
...
_FUNASRAPI FUNASR_RESULT FunASRInfer(FUNASR_HANDLE handle,
                                     const char* sz_filename, FUNASR_MODE mode,
                                     QM_CALLBACK fn_callback,
                                     int sampling_rate) {
  funasr::Model* recog_obj = (funasr::Model*)handle;
  if (!recog_obj) return nullptr;
  ...
  if (funasr::is_target_file(sz_filename, "wav")) {
    ...
    if (!audio.LoadWav(sz_filename, &sampling_rate_)) return nullptr;
  } else if (funasr::is_target_file(sz_filename, "pcm")) {
    if (!audio.LoadPcmwav(sz_filename, &sampling_rate)) return nullptr;
  } else {
    if (!audio.FfmpegLoad(sz_filename)) return nullptr;
  }
...
// APIs for VAD Infer
_FUNASRAPI FUNASR_RESULT FsmnVadInferBuffer(FUNASR_HANDLE handle,
                                            const char* sz_buf, int n_len,
                                            QM_CALLBACK fn_callback,
                                            bool input_finished,
                                            int sampling_rate,
                                            std::string wav_format) {
  funasr::VadModel* vad_obj = (funasr::VadModel*)handle;
  if (!vad_obj) return nullptr;
...
_FUNASRAPI FUNASR_RESULT FsmnVadInfer(FUNASR_HANDLE handle,
...
// APIs for PUNC Infer
_FUNASRAPI FUNASR_RESULT CTTransformerInfer(
    FUNASR_HANDLE handle, const char* sz_sentence, FUNASR_MODE mode,
    QM_CALLBACK fn_callback, PUNC_TYPE type, FUNASR_RESULT pre_result) {
  funasr::PuncModel* punc_obj = (funasr::PuncModel*)handle;
  if (!punc_obj) return nullptr;

  FUNASR_RESULT p_result = nullptr;
  if (type == PUNC_OFFLINE) {
    p_result = (FUNASR_RESULT) new funasr::FUNASR_PUNC_RESULT;
    ((funasr::FUNASR_PUNC_RESULT*)p_result)->msg =
        punc_obj->AddPunc(sz_sentence);
  } else if (type == PUNC_ONLINE) {
    if (!pre_result)
      p_result = (FUNASR_RESULT) new funasr::FUNASR_PUNC_RESULT;
    else
      p_result = pre_result;
    ((funasr::FUNASR_PUNC_RESULT*)p_result)->msg = punc_obj->AddPunc(
        sz_sentence, ((funasr::FUNASR_PUNC_RESULT*)p_result)->arr_cache);
  } else {
...
_FUNASRAPI FUNASR_RESULT FunOfflineInferBuffer(
    FUNASR_HANDLE handle, const char* sz_buf, int n_len, FUNASR_MODE mode,
    QM_CALLBACK fn_callback, const std::vector<std::vector<float>>& hw_emb,
    int sampling_rate, std::string wav_format, bool itn,
    FUNASR_DEC_HANDLE dec_handle, std::string svs_lang, bool svs_itn) {
  funasr::OfflineStream* offline_stream = (funasr::OfflineStream*)handle;
  if (!offline_stream) return nullptr;
  ...
  while (audio.FetchDynamic(buff, len, flag, start_time, batch_size, batch_in) >
         0) {
    // dec reset
    ...
    if (offline_stream->GetModelType() == MODEL_SVS) {
      msg_batch = (offline_stream->asr_handle)
                      ->Forward(buff, len, true, svs_lang, svs_itn, batch_in);
    } else {
      msg_batch = (offline_stream->asr_handle)
                      ->Forward(buff, len, true, hw_emb, dec_handle, batch_in);
    }
    for (int idx = 0; idx < batch_in; idx++) {
      ...
      if (msg_idx < index_vector.size()) {
        ...
        LOG(ERROR) << "msg_idx: " << msg_idx << " is out of range "
                   << index_vector.size();
      }
    }
  ...
  for (int idx = 0; idx < msgs.size(); idx++) {
    ...
    if (msg_vec.size() > 1) {
      ...
      for (size_t i = 0; i + 1 < msg_stamp.size(); i += 2) {
        ...
        cur_stamp += "[" + std::to_string((int)(1000 * begin)) + "," +
                     std::to_string((int)(1000 * end)) + "],";
      }
    }
  }
  if (cur_stamp != "[") {
  ...
  if (offline_stream->UsePunc()) {
    string punc_res =
        (offline_stream->punc_handle)->AddPunc((p_result->msg).c_str(), lang);
  ...
  if (offline_stream->UseITN() && itn) {
    ...
    if (!(p_result->stamp).empty()) {
      std::string new_stamp =
          funasr::TimestampSmooth(p_result->msg, msg_itn, p_result->stamp);
      if (!new_stamp.empty()) {
...
#endif
  if (!(p_result->stamp).empty()) {
    p_result->stamp_sents =
        funasr::TimestampSentence(p_result->msg, p_result->stamp);
  }
  return p_result;
}

_FUNASRAPI FUNASR_RESULT FunOfflineInfer(
    FUNASR_HANDLE handle, const char* sz_filename, FUNASR_MODE mode,
    QM_CALLBACK fn_callback, const std::vector<std::vector<float>>& hw_emb,
    int sampling_rate, bool itn, FUNASR_DEC_HANDLE dec_handle) {
  funasr::OfflineStream* offline_stream = (funasr::OfflineStream*)handle;
  if (!offline_stream) return nullptr;
...
// #if !defined(__APPLE__)
_FUNASRAPI const std::vector<std::vector<float>> CompileHotwordEmbedding(
    FUNASR_HANDLE handle, std::string& hotwords, ASR_TYPE mode) {
  if (mode == ASR_OFFLINE) {
    funasr::OfflineStream* offline_stream = (funasr::OfflineStream*)handle;
    std::vector<std::vector<float>> emb;
    if (!offline_stream) return emb;
    return (offline_stream->asr_handle)->CompileHotwordEmbedding(hotwords);
  } else if (mode == ASR_TWO_PASS) {
    funasr::TpassStream* tpass_stream = (funasr::TpassStream*)handle;
    std::vector<std::vector<float>> emb;
    if (!tpass_stream) return emb;
...
_FUNASRAPI FUNASR_RESULT FunTpassInferBuffer(
    FUNASR_HANDLE handle, FUNASR_HANDLE online_handle, const char* sz_buf,
    int n_len, std::vector<std::vector<std::string>>& punc_cache,
    bool input_finished, int sampling_rate, std::string wav_format,
    ASR_TYPE mode, const std::vector<std::vector<float>>& hw_emb, bool itn,
    FUNASR_DEC_HANDLE dec_handle, std::string svs_lang, bool svs_itn) {
  funasr::TpassStream* tpass_stream = (funasr::TpassStream*)handle;
  funasr::TpassOnlineStream* tpass_online_stream =
      (funasr::TpassOnlineStream*)online_handle;
  if (!tpass_stream || !tpass_online_stream) return nullptr;

  funasr::VadModel* vad_online_handle =
      (tpass_online_stream->vad_online_handle).get();
  if (!vad_online_handle) return nullptr;

  funasr::Audio* audio =
      ((funasr::FsmnVadOnline*)vad_online_handle)->audio_handle.get();

  funasr::Model* asr_online_handle =
      (tpass_online_stream->asr_online_handle).get();
  if (!asr_online_handle) return nullptr;
  int chunk_len = ((funasr::ParaformerOnline*)asr_online_handle)->chunk_len;

  funasr::Model* asr_handle = (tpass_stream->asr_handle).get();
  if (!asr_handle) return nullptr;

  funasr::PuncModel* punc_online_handle =
      (tpass_stream->punc_online_handle).get();
  if (!punc_online_handle) return nullptr;

  if (wav_format == "pcm" || wav_format == "PCM") {
    if (!audio->LoadPcmwavOnline(sz_buf, n_len, &sampling_rate)) return nullptr;
  } else {
  ...
  while (audio->FetchChunck(frame) > 0) {
    string msg =
        (asr_online_handle)->Forward(frame->data, frame->len, frame->is_final);
    if (mode == ASR_ONLINE) {
      ...
      if (frame->is_final) {
        ...
        string msg_punc =
            punc_online_handle->AddPunc(online_msg.c_str(), punc_cache[0]);
  ...
  while (audio->FetchTpass(frame) > 0) {
    ...
    if (tpass_stream->GetModelType() == MODEL_SVS) {
      msgs = (tpass_stream->asr_handle)
                 ->Forward(buff, len, true, svs_lang, svs_itn, 1);
    } else {
      msgs = (tpass_stream->asr_handle)
                 ->Forward(buff, len, true, hw_emb, dec_handle, 1);
    }
    string msg = msgs.size() > 0 ? msgs[0] : "";
    std::vector<std::string> msg_vec =
        funasr::SplitStr(msg, " | ");  // split with timestamp
    if (msg_vec.size() == 0) {
      continue;
    }
    msg = msg_vec[0];
    ...
    if (msg_vec.size() > 1) {
      ...
      for (size_t i = 0; i + 1 < msg_stamp.size(); i += 2) {
        float begin =
            std::stof(msg_stamp[i]) + float(frame->global_start) / 1000.0;
        float end =
            std::stof(msg_stamp[i + 1]) + float(frame->global_start) / 1000.0;
        cur_stamp += "[" + std::to_string((int)(1000 * begin)) + "," +
                     std::to_string((int)(1000 * end)) + "],";
      }
    }
    ...
    if (tpass_stream->GetModelType() == MODEL_PARA) {
      ...
      if (tpass_stream->UseITN() && itn) {
        ...
        // TimestampSmooth
        if (!(p_result->stamp).empty()) {
          std::string new_stamp = funasr::TimestampSmooth(
              p_result->tpass_msg, msg_itn, p_result->stamp);
          if (!new_stamp.empty()) {
            p_result->stamp = new_stamp;
          }
        }
    ...
    if (!(p_result->stamp).empty()) {
      p_result->stamp_sents =
          funasr::TimestampSentence(p_result->tpass_msg, p_result->stamp);
    }
    if (frame != nullptr) {
      delete frame;
      frame = nullptr;
...
_FUNASRAPI const char* CTTransformerGetResult(FUNASR_RESULT result,
                                              int n_index) {
...
_FUNASRAPI vector<std::vector<int>>* FsmnVadGetResult(FUNASR_RESULT result,
                                                      int n_index) {
...
_FUNASRAPI void FunASRReset(FUNASR_HANDLE handle,
                            FUNASR_DEC_HANDLE dec_handle) {
  funasr::Model* recog_obj = (funasr::Model*)handle;
  recog_obj->StartUtterance();
  funasr::WfstDecoder* wfst_decoder = (funasr::WfstDecoder*)dec_handle;
  if (wfst_decoder) wfst_decoder->StartUtterance();
}

_FUNASRAPI void FunOfflineReset(FUNASR_HANDLE handle,
                                FUNASR_DEC_HANDLE dec_handle) {
...
_FUNASRAPI void FunTpassOnlineUninit(FUNASR_HANDLE handle) {
  funasr::TpassOnlineStream* tpass_online_stream =
      (funasr::TpassOnlineStream*)handle;

  if (!tpass_online_stream) return;

  delete tpass_online_stream;
}

_FUNASRAPI FUNASR_DEC_HANDLE FunASRWfstDecoderInit(FUNASR_HANDLE handle,
                                                   int asr_type,
                                                   float glob_beam,
                                                   float lat_beam,
                                                   float am_scale) {
  ...
  if (asr_type == ASR_OFFLINE) {
    funasr::OfflineStream* offline_stream = (funasr::OfflineStream*)handle;
    auto paraformer =
        dynamic_cast<funasr::Paraformer*>(offline_stream->asr_handle.get());
    if (paraformer != nullptr) {
      if (paraformer->lm_) {
        mm = new funasr::WfstDecoder(
            paraformer->lm_.get(), paraformer->GetPhoneSet(),
            paraformer->GetLmVocab(), glob_beam, lat_beam, am_scale);
      }
      return mm;
    }
#ifdef USE_GPU
    auto paraformer_torch = dynamic_cast<funasr::ParaformerTorch*>(
        offline_stream->asr_handle.get());
    if (paraformer_torch != nullptr) {
      if (paraformer_torch->lm_) {
        mm = new funasr::WfstDecoder(
            paraformer_torch->lm_.get(), paraformer_torch->GetPhoneSet(),
            paraformer_torch->GetLmVocab(), glob_beam, lat_beam, am_scale);
      }
      return mm;
    }
#endif
    ...
    funasr::TpassStream* tpass_stream = (funasr::TpassStream*)handle;
    auto paraformer =
        dynamic_cast<funasr::Paraformer*>(tpass_stream->asr_handle.get());
    if (paraformer != nullptr) {
      if (paraformer->lm_) {
        mm = new funasr::WfstDecoder(
            paraformer->lm_.get(), paraformer->GetPhoneSet(),
...
#ifdef USE_GPU
    auto paraformer_torch =
        dynamic_cast<funasr::ParaformerTorch*>(tpass_stream->asr_handle.get());
    if (paraformer_torch != nullptr) {
      if (paraformer_torch->lm_) {
        mm = new funasr::WfstDecoder(
            paraformer_torch->lm_.get(), paraformer_torch->GetPhoneSet(),
...
_FUNASRAPI void FunASRWfstDecoderUninit(FUNASR_DEC_HANDLE handle) {
  ...
  if (!wfst_decoder) return;
  delete wfst_decoder;
}

_FUNASRAPI void FunWfstDecoderLoadHwsRes(FUNASR_DEC_HANDLE handle, int inc_bias,
...
_FUNASRAPI FUNASR_HANDLE
CampplusInit(std::map<std::string, std::string>& model_path, int thread_num) {
  funasr::CAMPPlusModel* model =
      funasr::CreateCAMPPlusModel(model_path, thread_num);
  return model;
}
...
_FUNASRAPI std::vector<float> CampplusExtractEmbedding(FUNASR_HANDLE handle,
...
_FUNASRAPI FUNASR_HANDLE SpeakerDiarizationInit(
    FUNASR_HANDLE campplus_handle, std::map<std::string, std::string>& config) {
  funasr::CAMPPlusModel* campplus_model =
      (funasr::CAMPPlusModel*)campplus_handle;
  if (!campplus_model) {
    LOG(ERROR) << "CAMPPlus model handle is null";
    return nullptr;
  }
  funasr::SpeakerDiarization* diarization =
      funasr::CreateSpeakerDiarization(campplus_model, config);
  return diarization;
}
...
_FUNASRAPI const char* SpeakerDiarizationProcess(
    FUNASR_HANDLE handle,
  ...
  std::vector<funasr::SpeakerSegment> segments =
      diarization->Process(vad_segments, sample_rate);
  ...
  for (size_t i = 0; i < segments.size(); ++i) {
    if (i > 0) json_result += ",";
    char buf[128];
    snprintf(buf, sizeof(buf), "[%.2f,%.2f,%d]", segments[i].start_time,
             segments[i].end_time, segments[i].speaker_id);
    json_result += buf;
  }
  json_result += "]";
