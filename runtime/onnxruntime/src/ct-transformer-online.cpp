...
void CTTransformerOnline::InitPunc(const std::string &punc_model,
                                   const std::string &punc_config,
  ...
  try {
    ...
    LOG(INFO) << "CUDA Execution Provider enabled for online punctuation model";
  ...
  try {
    m_session = std::make_unique<Ort::Session>(
        env_, ORTSTRING(punc_model).c_str(), session_options);
...
string CTTransformerOnline::AddPunc(const char *sz_input,
  ...
  if ((strText.size() > 0 and !(strText[strText.size() - 1] & 0x80)) &&
  ...
  vector<string> sentence_punc_list, sentence_words_list,
      sentence_punc_list_out;  // sentence_words_list = []

  int nSkipNum = 0;
  int nDiff = 0;
  for (size_t i = 0; i < InputData.size(); i += TOKEN_LEN) {
    nDiff = (i + TOKEN_LEN) < InputData.size()
                ? (0)
                : (i + TOKEN_LEN - InputData.size());
    vector<int32_t> InputIDs(InputData.begin() + i,
                             InputData.begin() + i + (TOKEN_LEN - nDiff));
    vector<string> InputStr(strOut.begin() + i,
                            strOut.begin() + i + (TOKEN_LEN - nDiff));
    InputIDs.insert(InputIDs.begin(), RemainIDs.begin(),
                    RemainIDs.end());  // RemainIDs+InputIDs;
    InputStr.insert(InputStr.begin(), RemainStr.begin(),
                    RemainStr.end());  // RemainStr+InputStr;
    ...
    if (nCurBatch < nTotalBatch - 1)  // not the last minisetence
    {
      ...
      for (int nIndex = Punction.size() - 2; nIndex > 0; nIndex--) {
        if (m_tokenizer.Id2Punc(Punction[nIndex]) ==
                m_tokenizer.Id2Punc(PERIOD_INDEX) ||
            m_tokenizer.Id2Punc(Punction[nIndex]) ==
                m_tokenizer.Id2Punc(QUESTION_INDEX)) {
          nSentEnd = nIndex;
          break;
        }
        if (nLastCommaIndex < 0 && m_tokenizer.Id2Punc(Punction[nIndex]) ==
                                       m_tokenizer.Id2Punc(COMMA_INDEX)) {
          nLastCommaIndex = nIndex;
        }
      }
      if (nSentEnd < 0 && InputStr.size() > CACHE_POP_TRIGGER_LIMIT &&
          nLastCommaIndex > 0) {
      ...
      InputStr.assign(InputStr.begin(),
                      InputStr.begin() + (nSentEnd + 1));  // minit_sentence
    ...
    sentence_words_list.insert(sentence_words_list.end(), InputStr.begin(),
                               InputStr.end());

    new_mini_sentence_punc.insert(new_mini_sentence_punc.end(),
                                  Punction.begin(), Punction.end());
  }
  vector<string> WordWithPunc;
  for (int i = 0; i < sentence_words_list.size();
       i++)  // for i in range(0, len(sentence_words_list)):
  {
    if (!(sentence_words_list[i][0] & 0x80) &&
        (i + 1) < sentence_words_list.size() &&
        !(sentence_words_list[i + 1][0] & 0x80)) {
  ...
  sentenceOut.insert(sentenceOut.end(), WordWithPunc.begin(),
                     WordWithPunc.end());  //
  nSentEnd = -1;
  for (int i = sentence_punc_list.size() - 2; i > 0; i--) {
    if (new_mini_sentence_punc[i] == PERIOD_INDEX ||
        new_mini_sentence_punc[i] == QUESTION_INDEX) {
      nSentEnd = i;
      break;
    }
  }
  arr_cache.assign(sentence_words_list.begin() + (nSentEnd + 1),
                   sentence_words_list.end());

  if (sentenceOut.size() > 0 &&
      m_tokenizer.IsPunc(sentenceOut[sentenceOut.size() - 1])) {
    sentenceOut.assign(sentenceOut.begin(), sentenceOut.end() - 1);
    sentence_punc_list_out[sentence_punc_list_out.size() - 1] =
        m_tokenizer.Id2Punc(NOTPUNC_INDEX);
  }
  return accumulate(sentenceOut.begin(), sentenceOut.end(), string(""));
}

vector<int> CTTransformerOnline::Infer(vector<int32_t> input_data,
                                       int nCacheSize) {
  ...
  Ort::Value onnx_input = Ort::Value::CreateTensor(
      m_memoryInfo, input_data.data(), input_data.size() * sizeof(int32_t),
  ...
  Ort::Value onnx_text_lengths = Ort::Value::CreateTensor<int32_t>(
      m_memoryInfo, text_lengths.data(), text_lengths.size(),
  ...
  Ort::Value onnx_sub_mask = Ort::Value::CreateTensor<float>(
      m_memoryInfo, arVadMask.data(), arVadMask.size(), SubMask_Dim.data(),
  ...
  try {
    auto outputTensor = m_session->Run(
...
void CTTransformerOnline::VadMask(int nSize, int vad_pos,
                                  vector<float> &Result) {
  Result.resize(0);
...
void CTTransformerOnline::Triangle(int text_length, vector<float> &Result) {
  Result.resize(0);
  Result.assign(text_length * text_length,
                1);  // generate a zeros: text_length x text_length
