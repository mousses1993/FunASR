...
class EncodeConverter {
 ...
 public:
  ...
  static size_t Utf16ToUtf8(const U16CHAR_T* pu16, size_t ilen, U8CHAR_T* pu8,
                            size_t olen);
  ...
  static std::vector<std::string> MergeEnglishWord(
      std::vector<std::string>& str_vec_input, std::vector<int>& merge_mask);
  static size_t Utf8ToCharset(const std::string& input,
  ...
  static std::string UTF8ToLocaleAnsi(const std::string& strUTF8) {
    ...
    len = WideCharToMultiByte(CP_ACP, 0, (LPCWCH)wszGBK, -1, nullptr, 0,
                              nullptr, nullptr);
    char* szGBK = new char[len + 1];
    memset(szGBK, 0, len + 1);
    WideCharToMultiByte(CP_ACP, 0, (LPCWCH)wszGBK, -1, szGBK, len, nullptr,
                        nullptr);
