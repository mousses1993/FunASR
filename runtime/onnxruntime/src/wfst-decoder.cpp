...
namespace funasr {
WfstDecoder::WfstDecoder(fst::Fst<fst::StdArc>* lm, PhoneSet* phone_set,
                         Vocab* vocab, float glob_beam, float lat_beam,
                         float am_scale)
    : dec_opts_(glob_beam, lat_beam, am_scale),
      decodable_(dec_opts_.acoustic_scale),
      lm_(lm),
...
string WfstDecoder::FinalizeDecode(bool is_stamp, std::vector<float> us_alphas,
  ...
  if (cur_token_ > 0) {
    ...
    if (!is_stamp) {
      ...
      for (auto& word : char_list) {
        ...
        split_chars.insert(split_chars.end(), word2char.begin(),
                           word2char.end());
      }
      // std::vector<string> raw_char(char_list);
      TimestampOnnx(us_alphas, us_cif_peak, split_chars, res_str,
                    timestamp_list);
...
void WfstDecoder::LoadHwsRes(int inc_bias,
                             unordered_map<string, int>& hws_map) {
  try {
    if (!hws_map.empty()) {
      bias_lm_ =
          std::make_shared<BiasLm>(hws_map, inc_bias, *phone_set_, *vocab_);
