...
Model *CreateModel(std::map<std::string, std::string> &model_path,
  ...
  if (type == ASR_OFFLINE) {
    ...
    Model *mm;
    mm = new Paraformer();
    mm->InitAsr(am_model_path, am_cmvn_path, am_config_path, token_path,
                thread_num);
    return mm;
    ...
    Model *mm;
    mm = new Paraformer();
    mm->InitAsr(en_model_path, de_model_path, am_cmvn_path, am_config_path,
