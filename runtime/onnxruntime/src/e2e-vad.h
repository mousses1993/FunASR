 ...
 * Reserved. MIT License  (https://opensource.org/licenses/MIT) Contributed by
 * zhuzizyf(China Telecom).
 */
...
class WindowDetector {
 public:
  ...
  int GetWinSize() { return win_size_frame; }
  ...
  int FrameSizeMs() { return frame_size_ms; }
};
 ...
 public:
  E2EVadModel() {
    ...
    this->max_end_sil_frame_cnt_thresh =
        this->vad_opts.max_end_silence_time -
        this->vad_opts.speech_to_sil_time_thres;
 ...
 private:
  ...
  void PopDataToOutputBuf(int start_frm, int frm_cnt,
    ...
    if (last_frm_is_end_point) {
      int extra_sample = std::max(
          0, int(vad_opts.frame_length_ms * vad_opts.sample_rate / 1000 -
  ...
  void OnVoiceStart(int start_frame, bool fake_result = false) {
    ...
    if (!fake_result && vad_state_machine ==
                            VadStateMachine::kVadInStateStartPointNotDetected) {
