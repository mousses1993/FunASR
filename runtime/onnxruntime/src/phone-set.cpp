...
void PhoneSet::LoadPhoneSetFromYaml(const char* filename) {
  ...
  int id = 0;
  for (YAML::const_iterator it = myList.begin(); it != myList.end();
       ++it, id++) {
...
bool PhoneSet::Find(string phn_str) const { return phn2Id_.count(phn_str) > 0; }

int PhoneSet::GetBegSilPhnId() const { return String2Id(UNIT_BEG_SIL_SYMBOL); }

int PhoneSet::GetEndSilPhnId() const { return String2Id(UNIT_END_SIL_SYMBOL); }

int PhoneSet::GetBlkPhnId() const { return String2Id(UNIT_BLK_SYMBOL); }
