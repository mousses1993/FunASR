...
namespace funasr {
TpassStream::TpassStream(std::map<std::string, std::string>& model_path,
                         int thread_num) {
  // VAD model
  if (model_path.find(VAD_DIR) != model_path.end()) {
    ...
    if (model_path.find(VAD_QUANT) != model_path.end() &&
        model_path.at(VAD_QUANT) == "true") {
  ...
  // AM model
  if (model_path.find(OFFLINE_MODEL_DIR) != model_path.end() &&
      model_path.find(ONLINE_MODEL_DIR) != model_path.end()) {
    // 2pass
    ...
    if ((access(hw_compile_model_path.c_str(), F_OK) == 0) &&
        (access(seg_dict_path.c_str(), F_OK) ==
         0)) {  // if model_eb.onnx exist, hotword enabled
      enable_hotword = true;
    ...
    if (model_path.find(QUANTIZE) != model_path.end() &&
        model_path.at(QUANTIZE) == "true") {
      am_model_path =
          PathAppend(model_path.at(OFFLINE_MODEL_DIR), QUANT_MODEL_NAME);
      en_model_path =
          PathAppend(model_path.at(ONLINE_MODEL_DIR), QUANT_ENCODER_NAME);
      de_model_path =
          PathAppend(model_path.at(ONLINE_MODEL_DIR), QUANT_DECODER_NAME);
    }
    am_cmvn_path = PathAppend(model_path.at(ONLINE_MODEL_DIR), AM_CMVN_NAME);
    am_config_path =
        PathAppend(model_path.at(ONLINE_MODEL_DIR), AM_CONFIG_NAME);
    token_path = PathAppend(model_path.at(MODEL_DIR), TOKEN_PATH);

    asr_handle->InitAsr(am_model_path, en_model_path, de_model_path,
                        am_cmvn_path, am_config_path, token_path,
                        online_token_path, thread_num);
  } else {
  ...
  // Lm resource
  if (model_path.find(LM_DIR) != model_path.end() &&
      model_path.at(LM_DIR) != "") {
    ...
    if (access(lex_path.c_str(), F_OK) != 0) {
      LOG(ERROR) << "Lexicon.txt file is not exist, please use the latest "
                    "version. Skip load LM model.";
    } else {
  ...
  if (model_path.find(PUNC_DIR) != model_path.end()) {
    ...
    if (model_path.find(PUNC_QUANT) != model_path.end() &&
        model_path.at(PUNC_QUANT) == "true") {
    ...
    if (access(punc_model_path.c_str(), F_OK) != 0 ||
      ...
      punc_online_handle->InitPunc(punc_model_path, punc_config_path,
                                   token_path, thread_num);
      use_punc = true;
    }
  }
  ...
  if (model_path.find(ITN_DIR) != model_path.end()) {
    string itn_tagger_path =
        PathAppend(model_path.at(ITN_DIR), ITN_TAGGER_NAME);
    string itn_verbalizer_path =
        PathAppend(model_path.at(ITN_DIR), ITN_VERBALIZER_NAME);
...
TpassStream* CreateTpassStream(std::map<std::string, std::string>& model_path,
                               int thread_num) {
  TpassStream* mm;
