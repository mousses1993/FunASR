...
template <typename T>
Tensor<T>::Tensor(int a) : size{1, 1, 1, a} {
  alloc_buff();
}

template <typename T>
Tensor<T>::Tensor(int a, int b) : size{1, 1, a, b} {
  alloc_buff();
}

template <typename T>
Tensor<T>::Tensor(int a, int b, int c) : size{1, a, b, c} {
  alloc_buff();
}
...
// TODO:: fix it!!!!
template <typename T>
void Tensor<T>::concat(Tensor<T> *din, int dim) {
...
// TODO:: fix it!!!!
template <typename T>
void Tensor<T>::resize(int a, int b, int c, int d) {
  size[0] = a;
  size[1] = b;
  size[2] = c;
  size[3] = d;
...
template <typename T>
void Tensor<T>::add(float coe, Tensor<T> *in) {
  int i;
...
template <typename T>
void Tensor<T>::add(Tensor<T> *in1, Tensor<T> *in2) {
  int i;
  for (i = 0; i < buff_size; i++) {
...
template <typename T>
void Tensor<T>::reload(Tensor<T> *in) {
  memcpy(buff, in->buff, in->buff_size * sizeof(T));
}
...
template <typename T>
void Tensor<T>::dump(const char *mode) {
  FILE *fp;
