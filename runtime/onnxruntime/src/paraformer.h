...
class Paraformer : public Model {
   ...
   * End-to-End Speech Recognition https://arxiv.org/pdf/2206.08317.pdf
   */
 private:
  Vocab *vocab = nullptr;
 ...
 public:
  ...
  void InitAsr(const std::string &am_model, const std::string &am_cmvn,
               ...
               int thread_num);
  // online
  void InitAsr(const std::string &en_model, const std::string &de_model,
               const std::string &am_cmvn, const std::string &am_config,
               const std::string &token_file, int thread_num);
  // 2pass
  void InitAsr(const std::string &am_model, const std::string &en_model,
  ...
  string BeamSearch(WfstDecoder *&wfst_decoder, float *in, int n_len,
                    int64_t token_nums);
  string FinalizeDecode(WfstDecoder *&wfst_decoder, bool is_stamp = false,
