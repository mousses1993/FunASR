...
void CTokenizer::SetJiebaRes(cppjieba::DictTrie* dict,
...
void CTokenizer::JiebaInit(std::string punc_config) {
  if (seg_jieba) {
    std::string model_path = punc_config.substr(
        0, punc_config.length() - (sizeof(PUNC_CONFIG_NAME) - 1));
    ...
    try {
      jieba_dict_trie_ =
          new cppjieba::DictTrie(jieba_dict_file, jieba_userdict_file);
      LOG(INFO) << "Successfully load file from " << jieba_dict_file << ", "
                << jieba_userdict_file;
...
string CTokenizer::Id2Punc(int n_punc_id) { return m_id2punc[n_punc_id]; }
...
void CTokenizer::StrSplit(const string& str, const char split,
                          vector<string>& res) {
  if (str == "") {
    return;
  }
...
void CTokenizer::Tokenize(const char* str_info, vector<string>& str_out,
                          vector<int>& id_out) {
