...
CAMPPlusModel::~CAMPPlusModel() { session_.reset(); }

bool CAMPPlusModel::InitModel(const std::string& model_path,
                              const std::string& config_path, int thread_num) {
  if (initialized_) {
  ...
  try {
    ...
    try {
      ...
      LOG(WARNING) << "Failed to enable CUDA Execution Provider for CAMPPlus: "
    ...
    LOG(ERROR) << "ONNX Runtime error during CAMPPlusModel initialization: "
               << e.what();
    return false;
...
std::vector<float> CAMPPlusModel::ExtractEmbedding(const float* features,
  ...
  try {
    ...
    auto output_tensors = session_->Run(
        Ort::RunOptions{nullptr}, input_name_ptrs_.data(), &input_tensor, 1,
    ...
    LOG(ERROR) << "ONNX Runtime error during embedding extraction: "
               << e.what();
    return std::vector<float>();
  } catch (const std::exception& e) {
...
std::vector<std::vector<float>> CAMPPlusModel::ExtractEmbeddings(
  ...
  // If you need to extract embeddings from raw audio, use the following
  // pattern:
  ...
  //
  // This method is kept for API compatibility but may be removed in future
  // versions.
  ...
  for (const auto& segment : audio_segments) {
    ...
    if (audio_data.size() % SPEAKER_FBANK_DIM != 0) {
      LOG(WARNING) << "Audio data size not divisible by feature dimension. "
                   << "Expected pre-computed fbank features, got size: "
                   << audio_data.size();
    ...
    auto embedding =
        ExtractEmbedding(audio_data.data(), num_frames, SPEAKER_FBANK_DIM);
...
CAMPPlusModel* CreateCAMPPlusModel(
    const std::map<std::string, std::string>& model_path, int thread_num) {
