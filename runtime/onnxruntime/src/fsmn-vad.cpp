...
namespace funasr {
void FsmnVad::InitVad(const std::string &vad_model, const std::string &vad_cmvn,
                      const std::string &vad_config, int thread_num) {
  ...
  try {
    ...
    LOG(WARNING) << "Failed to enable CUDA Execution Provider for VAD: "
...
void FsmnVad::LoadConfigFromYaml(const char *filename) {
  ...
  try {
    ...
    this->vad_speech_noise_thres_ =
        post_conf["speech_noise_thres"].as<double>();
    ...
    fbank_opts_.frame_opts.frame_shift_ms =
        frontend_conf["frame_shift"].as<float>();
    fbank_opts_.frame_opts.frame_length_ms =
        frontend_conf["frame_length"].as<float>();
...
void FsmnVad::ReadModel(const char *vad_model) {
  try {
    vad_session_ = std::make_shared<Ort::Session>(env_, ORTCHAR(vad_model),
                                                  session_options_);
...
void FsmnVad::Forward(const std::vector<std::vector<float>> &chunk_feats,
  ...
  for (int i = 0; i < in_cache->size(); i++) {
    vad_inputs.emplace_back(std::move(Ort::Value::CreateTensor<float>(
        memory_info, (*in_cache)[i].data(), (*in_cache)[i].size(),
        cache_feats_shape, 4)));
  }
...
std::vector<std::vector<int>> FsmnVad::Infer(std::vector<float> &waves,
                                             bool input_finished) {
  std::vector<std::vector<float>> vad_feats;
  ...
  vad_segments =
      vad_scorer(vad_probs, waves, true, false, vad_silence_duration_,
