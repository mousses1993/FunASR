...
AudioFrame::AudioFrame(int len) : len(len) { start = 0; }
...
AudioFrame::AudioFrame(int start, int end, bool is_final)
    : start(start), end(end), is_final(is_final) {
  len = end - start;
}
AudioFrame::~AudioFrame() {
  if (data != nullptr) {
...
int AudioFrame::GetStart() { return start; }

int AudioFrame::GetLen() { return len; }
...
Audio::Audio(int data_type)
    : dest_sample_rate(MODEL_SAMPLE_RATE), data_type(data_type) {
  speech_buff = nullptr;
  speech_data = nullptr;
  align_size = 1360;
...
Audio::Audio(int model_sample_rate, int data_type)
    : dest_sample_rate(model_sample_rate), data_type(data_type) {
  speech_buff = nullptr;
  speech_data = nullptr;
  align_size = 1360;
...
Audio::Audio(int model_sample_rate, int data_type, int size)
    : dest_sample_rate(model_sample_rate), data_type(data_type) {
  speech_buff = nullptr;
  speech_data = nullptr;
...
void Audio::Disp() {
  LOG(INFO) << "Audio time is " << (float)speech_len / dest_sample_rate
            << " s. len is " << speech_len;
}

float Audio::GetTimeLen() { return (float)speech_len / dest_sample_rate; }

void Audio::WavResample(int32_t sampling_rate, const float *waveform,
                        int32_t n) {
  LOG(INFO) << "Creating a resampler: "
            << " in_sample_rate: " << sampling_rate << " output_sample_rate: "
            << static_cast<int32_t>(dest_sample_rate);
...
bool Audio::FfmpegLoad(const char *filename, bool copy2char) {
  ...
  SwrContext *swr_ctx = swr_alloc_set_opts(
      ...
      av_get_default_channel_layout(
          codecContext->channels),  // input channel layout
      ...
      if (avcodec_send_packet(codecContext, packet) >= 0) {
        while (avcodec_receive_frame(codecContext, frame) >= 0) {
          ...
          int out_samples = av_rescale_rnd(
              swr_get_delay(swr_ctx, codecContext->sample_rate) +
                  frame->nb_samples,
              dest_sample_rate, codecContext->sample_rate, AV_ROUND_DOWN);

          int resampled_size =
              out_samples * av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
          ...
          resampled_buffers.insert(resampled_buffers.end(),
                                   resampled_buffer.begin(),
                                   resampled_buffer.begin() + resampled_size);
        }
      }
    }
  ...
  if (copy2char) {
    ...
    memcpy((void *)speech_char, (const void *)resampled_buffers.data(),
           resampled_buffers.size());
  }
  ...
  if (speech_data) {
    ...
    for (int32_t i = 0; i < speech_len; ++i) {
      int16_t val = (int16_t)((resampled_buffers[2 * i + 1] << 8) |
                              resampled_buffers[2 * i]);
...
bool Audio::FfmpegLoad(const char *buf, int n_file_len) {
  ...
  AVIOContext *avio_ctx =
      avio_alloc_context((unsigned char *)buf_copy,  // buffer
...
bool Audio::LoadWav(const char *buf, int n_file_len, int32_t *sampling_rate) {
  ...
  *sampling_rate = header.sample_rate;
  ...
  if (speech_buff) {
    memset(speech_buff, 0, sizeof(int16_t) * speech_len);
    memcpy((void *)speech_buff, (const void *)(buf + WAV_HEADER_SIZE),
           speech_len * sizeof(int16_t));
...
bool Audio::LoadPcmwavOnline(const char *buf, int n_buf_len,
...
int Audio::FetchDynamic(float **&dout, int *&len, int *&flag,
                        float *&start_time, int batch_size, int &batch_in) {
  // compute batch size
...
void Audio::Split(OfflineStream *offline_stream) {
  ...
  vector<std::vector<int>> vad_segments =
      (offline_stream->vad_handle)->Infer(pcm_data);
...
void Audio::CutSplit(OfflineStream *offline_stream,
  ...
  std::unique_ptr<VadModel> vad_online_handle =
      make_unique<FsmnVadOnline>((FsmnVad *)(offline_stream->vad_handle).get());
  AudioFrame *frame;
  ...
  bool is_final = false;
  vector<std::vector<int>> vad_segments;
  for (int sample_offset = 0; sample_offset < speech_len;
       sample_offset += std::min(step, speech_len - sample_offset)) {
    ...
    std::vector<float> pcm_data(speech_data + sample_offset,
                                speech_data + sample_offset + step);
    vector<std::vector<int>> cut_segments =
        vad_online_handle->Infer(pcm_data, is_final);
    vad_segments.insert(vad_segments.end(), cut_segments.begin(),
                        cut_segments.end());
  }
  ...
  // sort
  {
    ...
    std::sort(index_vector.begin(), index_vector.end(),
              [&vad_frames](const int a, const int b) {
...
// 2pass
void Audio::Split(VadModel *vad_obj, int chunk_len, bool input_finished,
                  ASR_TYPE asr_mode) {
  AudioFrame *frame;
  ...
  std::vector<float> pcm_data(speech_data, speech_data + sp_len);
  vector<std::vector<int>> vad_segments =
      vad_obj->Infer(pcm_data, input_finished);
    ...
    if (speech_start != -1) {
      ...
      if (asr_mode != ASR_OFFLINE) {
        if (buff_len >= step) {
          ...
          memcpy(frame->data, all_samples.data() + start - offset,
                 step * sizeof(float));
    ...
    for (auto vad_segment : vad_segments) {
      ...
      if (speech_start_i != -1 && speech_end_i != -1) {
        ...
        if (speech_start == -1 || speech_offline_start == -1) {
          LOG(ERROR) << "Vad start is null while vad end is available. Set vad "
                        "start 0";
          speech_start = 0;
        }
        ...
        if (asr_mode != ASR_ONLINE) {
          ...
          memcpy(frame->data, all_samples.data() + offline_start - offset,
                 (end - offline_start) * sizeof(float));
          asr_offline_queue.push(frame);
          frame = nullptr;
        }
        ...
        if (asr_mode != ASR_OFFLINE) {
          if (buff_len > 0) {
            for (int sample_offset = 0; sample_offset < buff_len;
                 sample_offset += std::min(step, buff_len - sample_offset)) {
              bool is_final = false;
              ...
              frame->data = (float *)malloc(sizeof(float) * step);
              memcpy(frame->data,
                     all_samples.data() + start - offset + sample_offset,
                     step * sizeof(float));
              asr_online_queue.push(frame);
              frame = nullptr;
            }
