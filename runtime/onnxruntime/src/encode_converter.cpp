...
size_t EncodeConverter::Utf16ToUtf8(const U16CHAR_T* pu16, size_t ilen,
  ...
  */
  for (size_t i = 0;
       i < ilen && static_cast<int>(offset) < static_cast<int>(olen); i++) {
    sz = Utf16ToUtf8(pu16 + i, pu8 + offset);
    if (static_cast<int>(offset + static_cast<int>(sz)) <=
...
u8string EncodeConverter::Utf16ToUtf8(const u16string& u16str) {
  ...
  size_t len = Utf16ToUtf8(u16str.data(), u16str.length(), pu8, buflen);
...
size_t EncodeConverter::Utf8ToUtf16(const U8CHAR_T* pu8, U16CHAR_T* pu16) {
  size_t n = 0;
  if ((pu8[0] & 0xF0) == 0xE0) {
    if ((pu8[1] & 0xC0) == 0x80 && (pu8[2] & 0xC0) == 0x80) {
...
size_t EncodeConverter::Utf8ToUtf16(const U8CHAR_T* pu8, size_t ilen,
  ...
  else
  {
      ...
      {
          if ((pu8[i] & 0xF0) == 0xE0 || (pu8[i] & 0xE0) == 0xC0 || (pu8[i] &
  0x80) == 0x00) break; n++;
      }
  }
...
u16string EncodeConverter::Utf8ToUtf16(const u8string& u8str) {
  ...
  size_t len =
      Utf8ToUtf16(u8str.data(), u8str.length(), p16, u8str.length() + 1);
...
bool EncodeConverter::IsUTF8(const U8CHAR_T* pu8, size_t ilen) {
  ...
  size_t n = 0;
  for (i = 0; i < ilen; i += n) {
    if ((pu8[i] & 0xF0) == 0xE0 && (pu8[i + 1] & 0xC0) == 0x80 &&
        (pu8[i + 2] & 0xC0) == 0x80) {
      n = 3;
    } else if ((pu8[i] & 0xE0) == 0xC0 && (pu8[i + 1] & 0xC0) == 0x80) {
      n = 2;
...
bool EncodeConverter::IsAllAlphaAndDigit(const U8CHAR_T* pu8, size_t ilen) {
  ...
  for (size_t i = 0; i < ilen; i++) {
    if (!(pu8[i] > 0 &&
          (isalnum(pu8[i]) || isalpha(pu8[i]) || pu8[i] == '\''))) {
      return false;
    }
  }
  return true;
}
bool EncodeConverter::IsAllAlphaAndDigitAndBlank(const U8CHAR_T* pu8,
                                                 size_t ilen) {
  if (pu8 == nullptr || ilen <= 0) {
    return false;
  }
  for (size_t i = 0; i < ilen; i++) {
    if (!(pu8[i] > 0 && (isalnum(pu8[i]) || isalpha(pu8[i]) ||
                         isblank(pu8[i]) || pu8[i] == '\''))) {
      return false;
    }
  }
...
bool EncodeConverter::NeedAddTailBlank(std::string str) {
  ...
  if (IsAllAlpha(pu8, ilen) || IsAllAlphaAndPunct(pu8, ilen) ||
      IsAllAlphaAndDigit(pu8, ilen)) {
    return true;
  } else {
    return false;
  }
}
std::vector<std::string> EncodeConverter::MergeEnglishWord(
    std::vector<std::string>& str_vec_input, std::vector<int>& merge_mask) {
...
size_t EncodeConverter::Utf8ToCharset(const std::string& input,
                                      std::vector<std::string>& output) {
  std::string ch;
