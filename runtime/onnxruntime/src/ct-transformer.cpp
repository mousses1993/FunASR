...
void CTTransformer::InitPunc(const std::string &punc_model,
...
string CTTransformer::AddPunc(const char *sz_input, std::string language) {
  ...
  for (size_t i = 0; i < InputData.size(); i += TOKEN_LEN) {
    ...
    NewPunctuation.insert(NewPunctuation.end(), Punction.begin(),
                          Punction.end());
    vector<string> WordWithPunc;
    for (int i = 0; i < InputStr.size(); i++) {
      // if (i > 0 && !(InputStr[i][0] & 0x80) && (i + 1) <InputStr.size() &&
      // !(InputStr[i+1][0] & 0x80))// �м��Ӣ�ģ�
    ...
    NewString.insert(
        NewString.end(), WordWithPunc.begin(),
        WordWithPunc.end());  // new_mini_sentence += "".join(words_with_punc)
    NewSentenceOut = NewString;
    NewPuncOut = NewPunctuation;
    // last mini sentence
    if (nCurBatch == nTotalBatch - 1) {
      if (NewString[NewString.size() - 1] == m_tokenizer.Id2Punc(COMMA_INDEX) ||
          NewString[NewString.size() - 1] == m_tokenizer.Id2Punc(DUN_INDEX)) {
  ...
  if (language == "en-bpe") {
    ...
    for (size_t i = 0; i < chineseSymbols.size(); i++) {
      size_t pos = 0;
      while ((pos = strResult.find(chineseSymbols[i], pos)) !=
             std::string::npos) {
...
vector<int> CTTransformer::Infer(vector<int32_t> input_data) {
  ...
  Ort::Value onnx_input = Ort::Value::CreateTensor<int32_t>(
      m_memoryInfo, input_data.data(), input_data.size(), input_shape_.data(),
      input_shape_.size());
  ...
  Ort::Value onnx_text_lengths = Ort::Value::CreateTensor(
      m_memoryInfo, text_lengths.data(), text_lengths.size() * sizeof(int32_t),
      text_lengths_dim.data(), text_lengths_dim.size(),
