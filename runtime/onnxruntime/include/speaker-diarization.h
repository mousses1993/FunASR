...
constexpr float DEFAULT_SEGMENT_DURATION = 1.5f;    // Duration of each segment in seconds
constexpr float DEFAULT_SEGMENT_SHIFT = 0.75f;      // Shift between segments in seconds
...
constexpr float DEFAULT_MERGE_THRESHOLD = 0.78f;    // Cosine similarity threshold for merging
constexpr float DEFAULT_MIN_SEGMENT_DURATION = 0.7f; // Minimum segment duration in seconds
...
private:
     ...
     */
    std::vector<SpeakerSegment> SmoothResults(std::vector<SpeakerSegment>& segments);
