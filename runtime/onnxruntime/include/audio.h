...
class DLLAPI Audio {
 ...
 public:
  ...
  int FetchDynamic(float**& dout, int*& len, int*& flag, float*& start_time,
                   int batch_size, int& batch_in);
  void Padding();
  ...
  void Split(VadModel* vad_obj, int chunk_len, bool input_finished = true,
             ASR_TYPE asr_mode = ASR_TWO_PASS);
  float GetTimeLen();
  ...
  bool LoadPcmwavOnline(const char* buf, int n_file_len,
