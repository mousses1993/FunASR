...
void runReg(FUNASR_HANDLE asr_handle, vector<string> wav_list,
            vector<string> wav_ids, int audio_fs, float* total_length,
            long* total_time, int core_id, float glob_beam = 3.0f,
            float lat_beam = 3.0f, float am_sc = 10.0f, int fst_inc_wts = 20,
            string hotword_path = "") {
  ...
  FUNASR_DEC_HANDLE decoder_handle = FunASRWfstDecoderInit(
      asr_handle, ASR_OFFLINE, glob_beam, lat_beam, am_sc);
  ...
  std::vector<std::vector<float>> hotwords_embedding =
      CompileHotwordEmbedding(asr_handle, nn_hotwords_);

  // warm up
  for (size_t i = 0; i < 1; i++) {
    FUNASR_RESULT result =
        FunOfflineInfer(asr_handle, wav_list[0].c_str(), RASR_NONE, nullptr,
                        hotwords_embedding, audio_fs, true, decoder_handle);
    if (result) {
  ...
  while (true) {
    ...
    FUNASR_RESULT result =
        FunOfflineInfer(asr_handle, wav_list[i].c_str(), RASR_NONE, nullptr,
                        hotwords_embedding, audio_fs, true, decoder_handle);
...
int main(int argc, char* argv[]) {
  ...
  TCLAP::ValueArg<std::string> model_dir(
      "", MODEL_DIR,
      "the model path, which contains model.onnx, config.yaml, am.mvn", true,
      "", "string");
  ...
  TCLAP::ValueArg<std::string> vad_dir(
      "", VAD_DIR,
      "the vad model path, which contains model.onnx, vad.yaml, vad.mvn", false,
      "", "string");
  ...
  TCLAP::ValueArg<std::string> punc_dir(
      "", PUNC_DIR, "the punc model path, which contains model.onnx, punc.yaml",
      false, "", "string");
  ...
  TCLAP::ValueArg<std::string> lm_dir("", LM_DIR,
                                      "the lm model path, which contains "
                                      "compiled models: TLG.fst, config.yaml ",
                                      false, "", "string");
  ...
  TCLAP::ValueArg<std::int32_t> thread_num(
  ...
  TCLAP::SwitchArg use_gpu("", INFER_GPU,
                           "Whether to use GPU for inference, default is false",
                           false);
  ...
  FUNASR_HANDLE asr_handle =
      FunOfflineInit(model_path, 1, use_gpu_, batch_size_);
  ...
  for (int i = 0; i < rtf_threds; i++) {
    threads.emplace_back(thread(runReg, asr_handle, wav_list, wav_ids,
                                audio_fs.getValue(), &total_length, &total_time,
                                i, glob_beam, lat_beam, am_sc, value_bias,
                                hotword_path));
  }
