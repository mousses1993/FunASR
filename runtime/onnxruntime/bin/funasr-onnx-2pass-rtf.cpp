...
void runReg(FUNASR_HANDLE tpass_handle, std::vector<int> chunk_size,
            vector<string> wav_list, vector<string> wav_ids, int audio_fs,
            float* total_length, long* total_time, int core_id,
            ASR_TYPE asr_mode_, string nn_hotwords_, float glob_beam,
            float lat_beam, float am_scale, int inc_bias,
  ...
  FUNASR_DEC_HANDLE decoder_handle = FunASRWfstDecoderInit(
      tpass_handle, ASR_TWO_PASS, glob_beam, lat_beam, am_scale);
  ...
  std::vector<std::vector<float>> hotwords_embedding =
      CompileHotwordEmbedding(tpass_handle, nn_hotwords_, ASR_TWO_PASS);
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    for (int sample_offset = 0; sample_offset < buff_len;
      ...
      FUNASR_RESULT result = FunTpassInferBuffer(
          tpass_handle, tpass_online_handle, speech_buff + sample_offset, step,
          punc_cache, is_final, sampling_rate_, "pcm", (ASR_TYPE)asr_mode_,
          hotwords_embedding, true, decoder_handle);
      if (result) {
  ...
  while (true) {
    ...
    for (int sample_offset = 0; sample_offset < buff_len;
      ...
      if (result) {
        ...
        if (online_msg != "") {
          LOG(INFO) << "Thread: " << this_thread::get_id() << " " << wav_ids[i]
                    << " : " << online_msg;
        }
    ...
    if (asr_mode_ == 2) {
      LOG(INFO) << "Thread: " << this_thread::get_id() << " " << wav_ids[i]
                << " Final online  results " << " : " << online_res;
    }
    if (asr_mode_ == 1) {
      LOG(INFO) << "Thread: " << this_thread::get_id() << " " << wav_ids[i]
                << " Final online  results " << " : " << tpass_res;
    }
    if (asr_mode_ == 0 || asr_mode_ == 2) {
      LOG(INFO) << "Thread: " << this_thread::get_id() << " " << wav_ids[i]
                << " Final offline results " << " : " << tpass_res;
      if (time_stamp_res != "") {
        LOG(INFO) << "Thread: " << this_thread::get_id() << " " << wav_ids[i]
                  << " Final timestamp results " << " : " << time_stamp_res;
      }
    }
  }
  {
...
int main(int argc, char** argv) {
  ...
  TCLAP::ValueArg<std::string> offline_model_dir(
      "", OFFLINE_MODEL_DIR,
      "the asr offline model path, which contains model.onnx, config.yaml, "
      "am.mvn",
      true, "", "string");
  TCLAP::ValueArg<std::string> online_model_dir(
      "", ONLINE_MODEL_DIR,
      "the asr online model path, which contains model.onnx, decoder.onnx, "
      "config.yaml, am.mvn",
      true, "", "string");
  TCLAP::ValueArg<std::string> quantize(
      "", QUANTIZE,
      "true (Default), load the model of model.onnx in model_dir. If set true, "
      "load the model of model_quant.onnx in model_dir",
      false, "true", "string");
  TCLAP::ValueArg<std::string> vad_dir(
      "", VAD_DIR,
      "the vad online model path, which contains model.onnx, vad.yaml, vad.mvn",
      false, "", "string");
  TCLAP::ValueArg<std::string> vad_quant(
      "", VAD_QUANT,
      "true (Default), load the model of model.onnx in vad_dir. If set true, "
      "load the model of model_quant.onnx in vad_dir",
      false, "true", "string");
  TCLAP::ValueArg<std::string> punc_dir(
      "", PUNC_DIR,
      "the punc online model path, which contains model.onnx, punc.yaml", false,
      "", "string");
  TCLAP::ValueArg<std::string> punc_quant(
      "", PUNC_QUANT,
      "true (Default), load the model of model.onnx in punc_dir. If set true, "
      "load the model of model_quant.onnx in punc_dir",
      false, "true", "string");
  TCLAP::ValueArg<std::string> itn_dir(
      "", ITN_DIR,
      "the itn model(fst) path, which contains zh_itn_tagger.fst and "
      "zh_itn_verbalizer.fst",
      false, "", "string");
  TCLAP::ValueArg<std::string> lm_dir(
      "", LM_DIR,
      "the lm model path, which contains compiled models: TLG.fst, "
      "config.yaml, lexicon.txt ",
      false, "", "string");
  ...
  TCLAP::ValueArg<std::int32_t> fst_inc_wts("", FST_INC_WTS,
                                            "the fst hotwords incremental bias",
                                            false, 20, "int32_t");

  TCLAP::ValueArg<std::string> asr_mode("", ASR_MODE, "offline, online, 2pass",
                                        false, "2pass", "string");
  TCLAP::ValueArg<std::int32_t> onnx_thread("", "model-thread-num",
                                            "onnxruntime SetIntraOpNumThreads",
                                            false, 1, "int32_t");
  TCLAP::ValueArg<std::int32_t> thread_num_(
      "", THREAD_NUM, "multi-thread num for rtf", false, 1, "int32_t");
  TCLAP::ValueArg<std::string> wav_path(
      "", WAV_PATH,
      "the input could be: wav_path, e.g.: asr_example.wav; pcm_path, e.g.: "
      "asr_example.pcm; wav.scp, kaldi style wav list (wav_id \t wav_path)",
      true, "", "string");
  TCLAP::ValueArg<std::int32_t> audio_fs(
      "", AUDIO_FS, "the sample rate of audio", false, 16000, "int32_t");
  ...
  long modle_init_micros =
      ((seconds * 1000000) + end.tv_usec) - (start.tv_usec);
  LOG(INFO) << "Model initialization takes "
            << (double)modle_init_micros / 1000000 << " s";
  ...
  for (int i = 0; i < rtf_threds; i++) {
    threads.emplace_back(thread(runReg, tpass_hanlde, chunk_size, wav_list,
                                wav_ids, audio_fs.getValue(), &total_length,
                                &total_time, i, (ASR_TYPE)asr_mode_,
                                nn_hotwords_, glob_beam, lat_beam, am_sc,
                                fst_inc_wts.getValue(), hws_map));
  }
