...
int main(int argc, char** argv) {
  ...
  FUNASR_HANDLE asr_hanlde =
      FunOfflineInit(model_path, thread_num, use_gpu_, batch_size_);
  ...
  FUNASR_DEC_HANDLE decoder_handle = FunASRWfstDecoderInit(
      asr_hanlde, ASR_OFFLINE, glob_beam, lat_beam, am_sc);
  ...
  std::vector<std::vector<float>> hotwords_embedding =
      CompileHotwordEmbedding(asr_hanlde, nn_hotwords_);
  for (int i = 0; i < wav_list.size(); i++) {
    ...
    // gettimeofday(&start, nullptr);
    // FUNASR_RESULT result=FunOfflineInferBuffer(asr_hanlde, speech_buff,
    // buff_len, RASR_NONE, nullptr, hotwords_embedding, audio_fs.getValue(),
    // "pcm", true, decoder_handle); For debug:end

    FUNASR_RESULT result = FunOfflineInfer(
        asr_hanlde, wav_file.c_str(), RASR_NONE, nullptr, hotwords_embedding,
        audio_fs.getValue(), true, decoder_handle);
    gettimeofday(&end, nullptr);
