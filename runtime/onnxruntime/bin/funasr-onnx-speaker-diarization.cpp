...
int main(int argc, char** argv) {
  ...
  TCLAP::ValueArg<std::string> speaker_dir(
      "", SPEAKER_DIR,
      "the speaker model path (CAM++), which contains model.onnx", true, "",
      "string");
  TCLAP::ValueArg<std::string> speaker_quant(
      "", SPEAKER_QUANT,
      "true (Default), load the model of model.onnx in speaker_dir. If set "
      "true, load the model of model_quant.onnx",
      false, "true", "string");
  ...
  TCLAP::ValueArg<std::string> wav_path(
      "", WAV_PATH,
      "the input audio file (wav, pcm, or other formats supported by ffmpeg)",
      true, "", "string");
  TCLAP::ValueArg<std::int32_t> audio_fs(
      "", AUDIO_FS, "the sample rate of audio", false, 16000, "int32_t");
  TCLAP::ValueArg<std::string> output_file(
      "", "output", "output file path for diarization results", false, "",
      "string");

  // Speaker diarization parameters
  TCLAP::ValueArg<std::int32_t> min_speakers(
      "", "min-speakers", "minimum number of speakers", false, 1, "int32_t");
  TCLAP::ValueArg<std::int32_t> max_speakers(
      "", "max-speakers", "maximum number of speakers", false, 15, "int32_t");
  ...
  long model_init_micros =
      ((seconds * 1000000) + end.tv_usec) - (start.tv_usec);
  LOG(INFO) << "CAM++ model initialization takes "
            << (double)model_init_micros / 1000000 << " s";
  ...
  if (vad_dir.isSet()) {
    ...
    LOG(INFO) << "VAD model initialization takes "
              << (double)model_init_micros / 1000000 << " s";
  }
  ...
  FUNASR_HANDLE diar_handle =
      SpeakerDiarizationInit(campplus_handle, diar_config);
  if (!diar_handle) {
  ...
  if (vad_handle) {
    ...
    FUNASR_RESULT vad_result = FsmnVadInferBuffer(
        vad_handle, speech_buff, buff_len, nullptr, true, sampling_rate, "pcm");
  ...
  if (!vad_segments.empty()) {
    LOG(INFO) << "Running speaker diarization on " << vad_segments.size()
              << " segments...";
    gettimeofday(&start, nullptr);

    const char* diar_result =
        SpeakerDiarizationProcess(diar_handle, vad_segments, sampling_rate);

    gettimeofday(&end, nullptr);
    seconds = (end.tv_sec - start.tv_sec);
    long diar_time = ((seconds * 1000000) + end.tv_usec) - (start.tv_usec);
    LOG(INFO) << "Speaker diarization takes " << (double)diar_time / 1000000
              << " s";
