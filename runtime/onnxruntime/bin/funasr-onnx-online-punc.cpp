...
void splitString(vector<string>& strings, const string& org_string,
                 const string& seq) {
...
int main(int argc, char* argv[]) {
  ...
  TCLAP::ValueArg<std::string> model_dir(
      "", MODEL_DIR,
      "the punc model path, which contains model.onnx, punc.yaml", true, "",
      "string");
  TCLAP::ValueArg<std::string> quantize(
      ...
      false, "true", "string");
  TCLAP::ValueArg<std::string> txt_path(
      "", TXT_PATH, "txt file path, one sentence per line", true, "", "string");
  ...
  int thread_num = 1;
  FUNASR_HANDLE punc_hanlde =
      CTTransformerInit(model_path, thread_num, PUNC_ONLINE);
  ...
  for (auto& txt_str : txt_list) {
    ...
    for (auto& vad_str : vad_strs) {
      result = CTTransformerInfer(punc_hanlde, vad_str.c_str(), RASR_NONE,
                                  nullptr, PUNC_ONLINE, result);
      if (result) {
