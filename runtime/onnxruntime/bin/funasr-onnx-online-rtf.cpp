...
void runReg(FUNASR_HANDLE asr_handle, vector<string> wav_list,
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    for (int sample_offset = 0; sample_offset < buff_len;
      ...
      FUNASR_RESULT result =
          FunASRInferBuffer(online_handle, speech_buff + sample_offset, step,
                            RASR_NONE, nullptr, is_final, sampling_rate_);
      if (result) {
  ...
  while (true) {
    ...
    LOG(INFO) << "Thread: " << this_thread::get_id() << ", Final results "
              << wav_ids[i] << " : " << final_res;
  }
  {
...
int main(int argc, char* argv[]) {
  ...
  TCLAP::ValueArg<std::int32_t> thread_num(
      "", THREAD_NUM, "multi-thread num for rtf", true, 0, "int32_t");
