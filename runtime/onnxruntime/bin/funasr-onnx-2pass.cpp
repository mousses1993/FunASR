...
int main(int argc, char** argv) {
  ...
  FUNASR_DEC_HANDLE decoder_handle = FunASRWfstDecoderInit(
      tpass_handle, ASR_TWO_PASS, glob_beam, lat_beam, am_sc);
  ...
  for (int i = 0; i < wav_list.size(); i++) {
    ...
    if (asr_mode_ == 0 || asr_mode_ == 2) {
      ...
      if (time_stamp_res != "") {
        LOG(INFO) << wav_id << " Final timestamp results " << " : "
                  << time_stamp_res;
      }
    }
  }
  ...
  LOG(INFO) << "Model inference takes: " << (double)taking_micros / 1000000
            << " s";
  LOG(INFO) << "Model inference RTF: "
