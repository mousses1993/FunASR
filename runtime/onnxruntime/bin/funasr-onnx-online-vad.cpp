...
int main(int argc, char* argv[]) {
  ...
  TCLAP::ValueArg<std::string> model_dir(
      "", MODEL_DIR,
      "the vad model path, which contains model.onnx, vad.yaml, vad.mvn", true,
      "", "string");
  ...
  for (int i = 0; i < wav_list.size(); i++) {
    ...
    for (int sample_offset = 0; sample_offset < buff_len;
      ...
      FUNASR_RESULT result =
          FsmnVadInferBuffer(online_hanlde, speech_buff + sample_offset, step,
                             nullptr, is_final, sampling_rate_);
      gettimeofday(&end, nullptr);
