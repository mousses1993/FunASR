...
class KenLm {
 ...
 public:
  ...
  int Load(std::string kenlm_filename, std::string kaldi_symbol_table_filename,
  ...
  void SetStateToBeginOfSentence(State *s) const {
    model_->BeginSentenceWrite(s);
  }
  ...
  inline BaseFloat Score(const State *in_state, WordIndex word,
...
class KenLmDeterministicOnDemandFst
    : public fst::DeterministicOnDemandFst<Arc> {
 public:
  ...
  virtual StateId Start() { return bos_state_id_; }
  ...
  virtual bool GetArc(StateId s, Label label, Arc *oarc) {
    ...
    oarc->weight =
        Weight(-log_10_prob * M_LN10);  // KenLm log10() -> Kaldi ln()
