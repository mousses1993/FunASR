...
// This header contains declarations from various convenience functions that are
// called from binary-level programs such as gmm-decode-faster.cc,
// gmm-align-compiled.cc, and so on.
...
void AlignUtteranceWrapper(
    ...
    int64 *frame_count, BaseFloatVectorWriter *per_frame_acwt_writer = NULL);
...
bool DecodeUtteranceLatticeIncremental(
    LatticeIncrementalDecoderTpl<FST>
        &decoder,                   // not const but is really an input.
    DecodableInterface &decodable,  // not const but is really an input.
    const TransitionInformation &trans_model, const fst::SymbolTable *word_syms,
...
class DecodeUtteranceLatticeFasterClass {
 public:
  ...
  DecodeUtteranceLatticeFasterClass(
      ...
      const fst::SymbolTable *word_syms, const std::string &utt,
      BaseFloat acoustic_scale, bool determinize, bool allow_partial,
      Int32VectorWriter *alignments_writer, Int32VectorWriter *words_writer,
