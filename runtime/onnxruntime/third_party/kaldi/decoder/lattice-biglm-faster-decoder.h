...
class LatticeBiglmFasterDecoder {
 public:
  ...
  bool GetBestPath(fst::MutableFst<LatticeArc> *ofst,
    ...
    // true); fstprinter.Print(&std::cout, "standard output");
 ...
 private:
  ...
  void PruneForwardLinks(int32 frame, bool *extra_costs_changed,
    ...
    // token links_pruned is set to true if any link in any token was pruned
  ...
  void PruneForwardLinksFinal(int32 frame) {
    ...
    while (changed) {
      ...
      for (Token *tok = active_toks_[frame].toks; tok != NULL;
        ...
        // (score+final_prob) of this token, and the best such
        // (score+final_prob).
  ...
  // tokens don't yet have forward pointers), but we do all previous frames,
  // unless we know that we can safely ignore them because the frame after them
  // was unchanged. delta controls when it considers a cost to have changed
  // enough to continue going backward and propagating the change. for a larger
  // delta, we will recurse less far back
      ...
      for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state); !aiter.Done();
        ...
        if (arc_ref.ilabel == 0) {  // propagate nonemitting only...
          ...
          if (tot_cost < cutoff) {
            bool changed;
            PairId next_pair = ConstructPair(arc.nextstate, next_lm_state);
            Elem *e_new = FindOrAddToken(next_pair, frame, tot_cost, false,
                                         &changed);  // false: non-emit

            tok->links = new ForwardLink(e_new->val, 0, arc.olabel, graph_cost,
                                         0, tok->links);
