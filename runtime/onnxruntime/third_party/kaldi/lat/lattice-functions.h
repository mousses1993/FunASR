...
//    If there are acoustic costs on input-epsilon arcs or the final-prob in
//    'nbest' (and this should not normally be the case in situations where it
//    makes sense to call this function), they will be included to the cost of
//    the preceding input symbol, or the following input symbol for
//    input-epsilons encountered prior to any input symbol.  If 'nbest' has no
//    input symbols, 'per_frame_loglikes' will be set to the empty vector.
// **/
...
// and
// /// counts the time instance corresponding to each state. The times are
// returned
// /// in a vector of integers 'times' which is resized to have a size equal to
// the
// /// number of states in the lattice. The function also returns the maximum
// time
...
// /// be topologically sorted.  Returns length of the utterance in frames,
// which
...
// /// of the lattice.  The Posterior quantities contain pairs of
...
//
// // This function is something similar to LatticeForwardBackward(), but it is
// on
// // the CompactLattice lattice format. Also we only need the alpha in the
// forward
// // path, not the posteriors.
// bool ComputeCompactLatticeAlphas(const CompactLattice &lat,
//                                  std::vector<double> *alpha);
//
// // A sibling of the function CompactLatticeAlphas()... We compute the beta
// from
...
//
//
// /// Topologically sort the compact lattice if not already topologically
// sorted.
...
// (or
// /// final-prob strings) crossing any given frame.  Returns 1 for empty
// lattices.
...
// /// frame.  This can be used to reduce the size of the "very deep" portions
// of
// /// the lattice.
...
// arc/path.
// /// There is a frame error if a particular transition-id on a particular
// frame
// /// corresponds to a phone not matching transcription's alignment for that
// frame.
// /// This is used in "margin-inspired" discriminative training, esp. Boosted
// MMI.
// /// The TransitionModel is used to map transition-ids in the lattice
// /// input-side to phones; the phones appearing in
// /// "silence_phones" are treated specially in that we replace the frame error
// f
// /// (either zero or 1) for a frame, with the minimum of f or
// max_silence_error.
...
//
// /**
//    This function implements either the MPFE (minimum phone frame error) or
//    SMBR (state-level minimum bayes risk) forward-backward, depending on
//    whether "criterion" is "mpfe" or "smbr".  It returns the MPFE criterion of
//    SMBR criterion for this utterance, and outputs the posteriors (which may
//    be positive or negative) into "post".
//
...
//                         corresponds to a silence phones are treated
//                         specially. The behavior is determined by
//                         'one_silence_class' being false (traditional
//                         behavior) or true. Usually in our setup, several
//                         phones including the silence, vocalized noise,
//                         non-spoken noise and unk are treated as "silence
//                         phones"
...
//    @param [in] one_silence_class   Determines how the silence frames are
//    treated.
//                         Setting this to false gives the old traditional
//                         behavior, where the silence frames (according to
//                         num_ali) are treated as incorrect. However, this
//                         means that the insertions are not penalized by the
//                         objective. Setting this to true gives the new
//                         behaviour, where we treat silence as any other phone,
//                         except that all pdfs of silence phones are collapsed
//                         into a single class for the frame-error computation.
//                         This can possible reduce the insertions in the
//                         trained model. This is closer to the WER metric that
//                         we actually care about, since WER is generally
//                         computed after filtering out noises, but does
//                         penalize insertions.
...
//    contribution for the numerator and a negative one for the denominator.
//    This function is not actually used in our normal MMI training recipes,
//    where it's instead done using various command line programs that each do a
//    part of the job.  This function was written for use in neural-net MMI
//    training.
//
//    @param [in] trans    The transition model. Used to map the
//                         transition-ids to phones or pdfs.
//    @param [in] lat      The denominator lattice
//    @param [in] num_ali  The numerator alignment
//    @param [in] drop_frames   If "drop_frames" is true, it will not compute
//    any
//                         posteriors on frames where the num and den have
//                         disjoint pdf-ids.
//    @param [in] convert_to_pdf_ids   If "convert_to_pdfs_ids" is true, it will
//                         convert the output to be at the level of pdf-ids, not
//                         transition-ids.
//    @param [in] cancel   If "cancel" is true, it will cancel out any positive
//    and
//                         negative parts from the same transition-id (or
//                         pdf-id, if convert_to_pdf_ids == true).
...
// /// linear sequence (e.g. derived from lattice-1best), and that should have
// been
// /// processed so that the arcs in the CompactLattice align correctly with the
// /// word boundaries (e.g. by lattice-align-words).  It outputs 3 vectors of
// the
// /// same size, which give, for each word in the lattice (in sequence), the
// word
// /// label and the begin time and length in frames.  This is done even for
// zero
// /// (epsilon) words, generally corresponding to optional silence-- if you
// don't
...
// been
// /// processed so that the arcs in the CompactLattice align correctly with the
// /// word boundaries (e.g. by lattice-align-words).  It outputs 4 vectors of
// the
// /// same size, which give, for each word in the lattice (in sequence), the
// word
// /// label, the begin time and length in frames, and the pronunciation
// (sequence
...
//
//
// /// A form of the shortest-path/best-path algorithm that's specially coded
// for
...
// /// have unique histories. Arcs with posteriors larger than epsilon get
// splitted. void ExpandCompactLattice(const CompactLattice &clat,
//                           double epsilon,
...
// /// for a state is the cost of the best path from the start state to the
// state.
// /// The traceback state of this state is its predecessor state in the best
// path.
// /// The backward best cost for a state is the cost of the best path from the
// /// state to a final one. Its traceback state is the successor state in the
// best
...
// /// minimal list of hypotheses that covers a lattice, to the graph scores on
// the
// /// arcs. The list of hypotheses are generated by latbin/lattice-path-cover.
// typedef unordered_map<std::pair<int32, int32>, double, PairHasher<int32> >
// MapT; void AddNnlmScoreToCompactLattice(const MapT &nnlm_scores,
//                                   CompactLattice *clat);
//
...
//                                    CompactLattice *clat);
//
// /// This function *adds* the negated scores obtained from the Decodable
// object,
// /// to the acoustic scores on the arcs.  If you want to replace them, you
// should
// /// use ScaleCompactLattice to first set the acoustic scores to zero. Returns
// /// true on success, false on error (typically some kind of mismatched
// inputs). bool RescoreCompactLattice(DecodableInterface *decodable,
//                            CompactLattice *clat);
//
//
...
// /// computing probabilities on most frames where all the pdf-ids are the
// same.
// /// (it needs the transition-model to work out whether two transition-ids map
// to
// /// the same pdf-id, and it assumes that the lattice has transition-ids on
// it).
...
// /// The input labels, if nonzero, are interpreted as transition-ids or
// whatever
...
// /// CompactLattice format lattice. The first element (the one that
// corresponds
// /// to LM weight) in CompactLatticeWeight is used for composition.
// ///
// /// Note that the DeterministicOnDemandFst interface is not "const",
// therefore
...
// ///
// ///   @param [in] lat   Input lattice. Expected to be top-sorted. Otherwise
// the
// ///                     function will crash.
...
// ///                     transition-id) on a frame will be the same for all
// the
// ///                     occurences of the pdf-id in that frame.
// ///                     But if not, we will take the average of the acoustic
// ///                     scores. Hence, we store both the
// sum-of-acoustic-scores
// ///                     and the num-of-occurences of the transition-id in
// that
// ///                     frame.
...
// ///                      A map from the pair (frame-index, transition-id) to
// a
