...
///  for edit distance", Haihua Xu, Daniel Povey, Lidia Mangu and Jie Zhu,
///  Computer Speech and Language, 2011
/// This is a slightly more principled way to do Minimum Bayes Risk (MBR)
/// decoding than the standard "Confusion Network" method.  Note: MBR decoding
/// aims to minimize the expected word error rate, assuming the lattice encodes
/// the true uncertainty about what was spoken; standard Viterbi decoding gives
/// the most likely utterance, which corresponds to minimizing the expected
/// sentence error rate.
///
...
struct MinimumBayesRiskOptions {
  ...
  /// hypothesis to do MBR decoding (if false, our output is the MAP decoded
  /// output, but we output the stats too (i.e. the confidences)).
  bool decode_mbr;
  /// Boolean configuration parameter: if true, the 1-best path will 'keep' the
  /// <eps> bins,
  bool print_silence;

  MinimumBayesRiskOptions() : decode_mbr(true), print_silence(false) {}
  void Register(OptionsItf *opts) {
    opts->Register("decode-mbr", &decode_mbr,
                   "If true, do Minimum Bayes Risk "
                   "decoding (else, Maximum a Posteriori)");
    opts->Register("print-silence", &print_silence,
                   "Keep the inter-word '<eps>' "
                   "bins in the 1-best output (ctm, <eps> can be a 'silence' "
                   "or a 'deleted' word)");
  }
};
...
class MinimumBayesRisk {
 public:
  ...
  // Note that the default value of opts.decode_mbr is true. If you provide
  // 1-best hypothesis from MAP decoding, the output ctm from MBR decoding may
  // be mismatched with the provided <words> (<words> would be used as the
  // starting point of optimization).
  MinimumBayesRisk(const CompactLattice &clat, const std::vector<int32> &words,
                   MinimumBayesRiskOptions opts = MinimumBayesRiskOptions());
  // Uses the provided <words> as <R_> and <times> of bins instead of using the
  // lattice best path. Note that the default value of opts.decode_mbr is true.
  // If you provide 1-best hypothesis from MAP decoding, the output ctm from MBR
  // decoding may be mismatched with the provided <words> (<words> would be used
  // as the starting point of optimization).
  MinimumBayesRisk(const CompactLattice &clat, const std::vector<int32> &words,
  ...
  const std::vector<std::vector<std::pair<BaseFloat, BaseFloat> > > GetTimes()
      const {
 ...
 private:
  ...
  // the use of certain epsilon transitions in the edit-distance which would
  // cause words not to show up in the accumulated edit-distance statistics.
