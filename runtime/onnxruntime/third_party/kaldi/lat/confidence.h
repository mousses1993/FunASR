...
/// word-sequence; if n == 2 it outputs to "second_best_sentence" (if non-NULL)
/// the second best word-sequence (this may be useful for testing whether the
/// two best word sequences are somehow equivalent for the task at hand).  If
/// you need more information than this or want to look deeper inside the n-best
/// list, then look at the implementation of this function. This function
/// requires that distinct paths in "lat" have distinct word sequences; this
/// will automatically be the case if you generated "clat" in any normal way,
/// such as from a decoder, because a deterministic FST has this property. This
/// function assumes that any acoustic scaling you want to apply, has already
/// been applied.
...
/// lattice. It needs to determinize it first, but it does so in a "smart" way
/// that only generates about as many output paths as it needs.
